...
If ( lOpen := MyOpenSm0Ex() )
	...
	If lOpen
		...
		//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
			...
			Else
				RpcSetType(2)
				RpcSetEnv(aRecnoSM0[1,2], aRecnoSM0[1,3,1])
				lMsFinalAuto := .F.
						
				IncProc( dtoc( Date() )+" "+Time()+" "+"Atualizando Base de Dados: "+aRecnoSM0[1,3,1] )
				cTexto += "Atualizando Base de Dados: "+aRecnoSM0[1,3,1]+CHR(13)+CHR(10)
				AcaLog( cArquivo, dtoc( Date() )+" "+Time()+" "+"Atualizando Base de Dados: "+aRecnoSM0[1,3,1] )
				
				AtuBase(aRecnoSM0[1,2], aRecnoSM0[1,3,1])
