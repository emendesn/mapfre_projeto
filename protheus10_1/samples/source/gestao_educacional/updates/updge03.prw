...
If ( lOpen := MyOpenSm0Ex() )
	...
	If lOpen
		For nI := 1 To Len(aRecnoSM0)
			...
			//³Atualiza o dicionario de dados.³
			//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
			IncProc("Atualizando Dados da Tabela JD1...")
			cTexto += AtuBase(SM0->M0_CODIGO)
			RpcClearEnv()
			If !( lOpen := MyOpenSm0Ex() )
				Exit 
			EndIf 
		...
		For nI := 1 To Len(aRecnoSM0)
			...
			ProcRegua(8)
			IncProc("Analisando Dicionario de Arquivos SX2...")
			cTexto += GEAtuSX2()
...
*******************************************************************************************/
If SX2->(DbSeek("JD1")) 
	If !ALLTRIM(SX2->X2_UNICO) == "JD1_FILIAL+JD1_NUMRA+JD1_NUMREQ+JD1_CODCUR+JD1_PERLET+JD1_HABILI+JD1_DISCIP+JD1_ITEM"
		aAdd(aSX2,{	"JD1",; 								//Chave
					cPath,;									//Path
					...
					"Analise de Grade X Disciplinas",;		//Nome Port
					0,;										//Delete
					SX2->X2_MODO,;							//Modo - (C)Compartilhado ou (E)Exclusivo
					"",;									//TTS
					"",;									//Rotina
...
For i:= 1 To Len(aSX2)
	If !Empty(aSX2[i][1])
		...
		MsUnLock()
		cTexto += " Tabela SX2 Arquivo: " + aSX2[i,1]+cNome
...
±±ºPrograma  ³GetJD1ItemºAutor  ³Rafael Rodrigues    º Data ³ 26/Set/2006 º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³Identifica qual o proximo numero de item a ser utilizado na º±±
±±º          ³criacao de registros na tabela JD1.                         º±±
...
*/
Static Function GetJD1Item( kFilial, cNumRA, cCodCur, cPerLet, cHabili, cNumReq, cDiscip )
Local cRet		:= StrZero( 1, TamSX3("JD1_ITEM")[1] )
Local cQuery	:= ""
Local cAlias	:= CriaTrab(,.F.)

cQuery := "select max(JD1_ITEM) ITEM"
cQuery += "  from "+RetSQLName("JD1")
cQuery += " where JD1_FILIAL = '"+kFilial+"' and D_E_L_E_T_ = ' '"
cQuery += "   and JD1_NUMRA  = '"+cNumRA+"'"
cQuery += "   and JD1_DISCIP = '"+cDiscip+"'"
cQuery += "   and JD1_CODCUR = '"+cCodCur+"'"
cQuery += "   and JD1_PERLET = '"+cPerLet+"'"
cQuery += "   and JD1_HABILI = '"+cHabili+"'"
cQuery += "   and JD1_NUMREQ = '"+cNumReq+"'"

cQuery := ChangeQuery( cQuery )
dbUseArea( .T., 'TOPCONN', TCGenQry(,, cQuery), cAlias, .F., .F. )
(cAlias)->( DbGoTop() )
if (cAlias)->( !eof() )
	cRet := Soma1( (cAlias)->ITEM )
endif
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÚÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄ¿±±
±±³Fun‡…o    ³  AtuBase ³ Autor ³ Otacilio A. Junior   ³ Data ³ 03/Jun/08 ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄ´±±
±±³Descri‡…o ³ Ajuste na Tabela JD1 após a inlcusão do campos jd1_item    ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
±±³ Uso      ³ Atualizacao GE                                             ³±±
±±ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß*/
Static Function AtuBase(cCodemp)   
cTexto:= ""
nProc := 0
...
While SM0->(!Eof()) .and. SM0->M0_CODIGO == cCodEmp
	...
	nProc:= JD1->(LastRec())
	SetProc(nProc)
	cTexto+= "Atualizando JD1_ITEM...Empresa: "+cCodEmp+"Filial: "+SM0->M0_CODFIL+CHR(13)+CHR(10)
	While JD1->(!EOF()) .AND. JD1->JD1_FILIAL == SM0->M0_CODFIL
	    IncProc("Atualizando a Tabela JD1")
		RecLock("JD1",.F.)
		JD1->JD1_ITEM:= JD1->(GetJD1Item(JD1_FILIAL, JD1_NUMRA, JD1_CODCUR, JD1_PERLET, JD1_HABILI, JD1_NUMREQ, JD1_DISCIP ))
		JD1->(MsUnLock())  
		JD1->(DbSkip())
	End
	SM0->(dbSkip())
