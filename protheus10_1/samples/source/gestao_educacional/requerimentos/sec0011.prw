...
±±³Programador ³ Data   ³ BOPS ³  Motivo da Alteracao                     ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
±±³Fernando    ³23/07/07³103222³Alteração feita na gravação da JC5        ³±±
±±ÀÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
*/
...
Local lConvEmp	:= .F.  	// informa se a bolsa é conveniada a empresa
Local cQuery		:= ""                       
...
JC4->( DbSetOrder( 1 ) )
If JC4->( DbSeek( xFilial("JC4") + aRet[12]))
	If JC4->JC4_TPCONV == "2"
		lConvEmp := .T.
	Endif
Endif
...
if Len( aRet ) > 14
	if !Empty(aRet[15])
		...
		JC5->JC5_ITEM	:= "01"
		JC5->JC5_TIPBOL := aRet[12]
		JC5->JC5_CURSO := JCS->JCS_CURSO
		JC5->JC5_BOLSA :=  aRet[13]
		JC5->JC5_PERBOL := val(aRet[15])
		JC5->JC5_VLRBOL := val(aRet[16])
		JC5->JC5_DTVAL1 := ctod(aRet[17])
		JC5->JC5_DTVAL2 := ctod(aRet[18])
		JC5->JC5_PERDE := aRet[19]
		If lConvEmp
			JC5->JC5_CLIENT := cCodCli
		Endif
		JC5->JC5_LOJA := "01"
		JC5->JC5_MATR := aRet[20]
...
For nA := 1 To Val(JCS->JCS_SERIE)
	...
	//Alterado devido a nomenclatura das turmas ser diferente em cada periodo
	cTurma := Posicione( "JBO", 1, xFilial("JBO")+JCS->JCS_CURSO+cSerie+JCS->JCS_HABILI, "JBO_TURMA")	
	...
	If JBE->JBE_ATIVO <> "1"
		If TCCanOpen(RetSqlName("JJ2"))
			JJ2->( dbSetOrder( 1 ) )
	   		If JJ2->( dbSeek( xFilial( "JJ2" ) + JBE->JBE_NUMRA + JBE->JBE_CODCUR ) )
	   			While xFilial( "JJ2" )+ JBE->JBE_NUMRA + JBE->JBE_CODCUR  == JJ2->JJ2_FILIAL + JJ2->JJ2_NUMRA + JJ2->JJ2_CURSO .and. !JJ2->(Eof())
		   			RecLock( "JJ2", .F. )	
					JJ2->JJ2_STATUS  := "2"	
	   		   		JJ2->( MsUnlock() )
	...
	While !JCT->( eof() ) .and. JCT->JCT_NUMREQ == JCS->JCS_NUMREQ .and. JCT->JCT_PERLET == cSerie .And. JCT->JCT_HABILI == cHabili
		...
		JBL->( dbSetOrder(1) )	// JBL_FILIAL+JBL_CODCUR+JBL_PERLET+JBL_TURMA+JBL_CODDIS            
		 ...
		 //Alterado devido a nomenclatura das turmas ser diferente em cada periodo
		 cTurma := Posicione( "JBO", 1, xFilial("JBO")+JCS->JCS_CURSO+JCT->JCT_PERLET+JCS->JCS_HABILI, "JBO_TURMA" ) 
		
		IF JBL->( dbSeek(xFilial("JBL")+JCS->JCS_CURSO+JCT->JCT_PERLET+JCT->JCT_HABILI+cTurma+JCT->JCT_DISCIP) )
		
			While !JBL->( eof() ) .and. JBL->JBL_FILIAL+JBL->JBL_CODCUR+JBL->JBL_PERLET+JBL->JBL_HABILI+JBL->JBL_TURMA+JBL->JBL_CODDIS == ;
				xFilial("JBL") + JCS->JCS_CURSO + JCT->JCT_PERLET + JCT->JCT_HABILI + cTurma + JCT->JCT_DISCIP
				dbSelectArea("JC7")    
				JC7->( dbSetOrder(1) )         
				if JC7->(dbSeek(xFilial("JC7")+cNumRA+JBE->(JBE_CODCUR+JBE_PERLET+JBE_HABILI)+cTurma+JCT->(JCT_DISCIP)+JBL->(JBL_CODLOC+JBL_CODPRE+JBL_ANDAR+JBL_CODSAL+JBL_DIASEM+JBL_HORA1)) )
					JC7->( RecLock("JC7", .F.) )					
				else
					JC7->( RecLock("JC7", .T.) )
				endif
				...
				If JC7->( FieldPos( "JC7_SUBTUR" ) ) > 0 .and. JCS->( FieldPos( "JCS_SUBTUR" ) ) > 0
				   JC7->JC7_SUBTUR := JCS->JCS_SUBTUR
				Endif
				JC7->( MsUnLock() )
...
while (cArqTrb)->( !eof() )
	if JCO->( !dbSeek( xFilial("JCO")+padr(cNumRA,Tamsx3("JCO_NUMRA")[1])+JCS->JCS_CURSO+(cArqTrb)->( JCT_PERLET+JCT_HABILI+JCT_DISCIP ) ) )
	   	RecLock("JCO", .T.)
		JCO->JCO_FILIAL := xFilial("JCO")		
		JCO->JCO_NUMRA  := cNumRA
		...
		if lJCOJust .and. lJCTJust                                                                                          
			cMemoJCT := (cArqTrb)->( MSMM( JCT_MEMO1 ) )
			JCO->( MSMM(,TamSx3("JCO_JUSTIF")[1],,cMemoJCT,1,,,"JCO","JCO_MEMO1") )		// Justificativa de dispensa			
		endif
		JCO->( msUnlock() )
	endif
...
If lExistJI6
	...
	//³Varre a tabela JI3 buscando todos os componentes do curso³
	...
	While !JI3->(Eof()) .And. JI3->JI3_FILIAL+JI3->JI3_CODCUR == xFilial("JI3")+JCS->JCS_CURSO
		...
		If JI6->(!DBSeek(xFilial("JI6") + JA2->JA2_NUMRA + JI3->JI3_CODCUR + JI3->JI3_COMP))				
		RecLock("JI6", .T.)
		JI6->JI6_FILIAL := xFilial("JI6")
		JI6->JI6_NUMRA  := JA2->JA2_NUMRA
...
If lExistJI6
	For nA := 1 To Val(JCS->JCS_SERIE)
		...
		ACA060AtJI6(JCS->JCS_CURSO, cSerie, cHabili,JA2->JA2_NUMRA)				
    Next nA
...
/*JAH->(dbSetOrder(4))
if JAH->(dbSeek(xFilial('JAH')+ JAF->JAF_COD + JAF->JAF_VERSAO))
	If JAH->JAH_STATUS == "1"
		lRet := (Posicione( "JBK", 3, xFilial( "JBK" ) + "1" + JAH->JAH_CODIGO, "JBK_ATIVO" ) == "1")
	EndIf            
endif   */

cQry := " SELECT COUNT (*) TOTAL FROM " 
cQry +=   retsqlname('JAH') + " JAH, " + retsqlname('JBK') + " JBK"
cQry += " WHERE JAH.JAH_CURSO  = '" + JAF->JAF_COD + "'"
cQry += " AND JAH.JAH_VERSAO = '" + JAF->JAF_VERSAO + "'"
cQry += " AND JAH.JAH_STATUS = '1' "
cQry += " AND JAH.JAH_CODIGO = JBK.JBK_CODCUR "
cQry += " AND JBK.JBK_ATIVO = '1' "
cQry += " AND JAH.JAH_FILIAL = '" + xFilial('JAH') + "'"
cQry += " AND JBK.JBK_FILIAL = '" + xFIlial('JBK') + "'"
cQry += " AND JAH.D_E_L_E_T_ <> '*' "
cQry += " AND JBK.D_E_L_E_T_ <> '*' "
TCQuery cQry NEW ALIAS "JAHTMP"
dbSelectArea("JAHTMP")
If JAHTMP->TOTAL > 0
 	lRet := .T.
endif 
