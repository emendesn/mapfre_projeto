...
aAdd(aSX2,{	"JJJ",; 								//Chave
			cPath,;									//Path
			"JJJ"+cNome,;							//Nome do Arquivo
			"Controla bloq. de registros",;			//Nome Port
			"Controla bloq. de registros",;			//Nome Port
			"Controla bloq. de registros",;			//Nome Port
			0,;										//Delete
			"C",;									//Modo - (C)Compartilhado ou (E)Exclusivo
...
aAdd(aSX3,{	"JHW",;								//Arquivo
			...
			"Tipo do Controle",;				//Descricao
			"@!",;								//Picture
			"Pertence('12') .And. A655TipCon()",;//VALID
			cUsadoObr,;							//USADO
			"",;								//RELACAO
			"",;								//F3
			1,;									//NIVEL
...
aAdd(aSX3,{	"JHW",;								//Arquivo
			...
			"","","",;							//CONTEXT, OBRIGAT, VLDUSER
			"1=NAO CONTROLA;2=CONTROLA MINIMO;3=CONTROLA MAXIMO;4=CONTROLA AMBOS","1=NAO CONTROLA;2=CONTROLA MINIMO;3=CONTROLA MAXIMO;4=CONTROLA AMBOS","1=NAO CONTROLA;2=CONTROLA MINIMO;3=CONTROLA MAXIMO;4=CONTROLA AMBOS",;	//CBOX, CBOX SPA, CBOX ENG
			"",'GDFieldGet("JHW_TPCONT") == "2"',"","","","S"})	//PICTVAR, WHEN, INIBRW, SXG, FOLDER, PYME
...
aAdd(aSX3,{	"JAF",;								//Arquivo
			...
			"Codigo Modelo Curricular",;		//Descricao
			"@!",;								//Picture
			"Vazio() .or. (ExistCpo('JHV',M->JAF_CODMOD,1) .and. A210VlMod(M->JAF_CODMOD))",;	//VALID
			cUsadoOpc,;							//USADO
			"",;					   			//RELACAO
			"JHV",;								//F3
			1,;									//NIVEL
...
aAdd(aSX3,{	"JHZ",;								//Arquivo
			...
			"Disciplina Associada",;			//Descricao
			"@!",;								//Picture
			"ExistCpo('JAE',M->JHZ_CODDIS) .And. Ac210VlDis()",; //VALID
			cUsadoObr,;							//USADO
			"",;								//RELACAO
			"JAE",;								//F3
			1,;									//NIVEL
			...
			"","","",;							//CONTEXT, OBRIGAT, VLDUSER
			"","","",;							//CBOX, CBOX SPA, CBOX ENG
			"",'A210WhenMo(M->JAF_COD,M->JAF_VERSAO, "JHZ_CODDIS")',"","","","S"}) //PICTVAR, WHEN, INIBRW, SXG, FOLDER, PYME
...
aAdd(aSX3,{	"JHZ",;								//Arquivo
			...
			"Tipo do Componente",;				//Descricao
			"@!",;								//Picture
			"Pertence('12') .and. A210VlTpCom(M->JHZ_TIPCOM)",; //VALID
			cUsadoOpc,;							//USADO
			"'1'",;								//RELACAO
			"",;								//F3
			1,;									//NIVEL
...
aAdd(aSX3,{	"JHZ",;								//Arquivo
			...
			"Codigo Modelo Curricular",;		//Descricao
			"@!",;								//Picture
			"ExistCpo('JHV',M->JHZ_CODMOD,1) .and. A210VldMod(M->JHZ_CODMOD)",;	//VALID
			cUsadoObr,;							//USADO
			"JAF->JAF_CODMOD",;	            	//RELACAO
			"JHV",;								//F3
			1,;									//NIVEL
			cReservObr,;						//RESERV
			...
			"","","",;							//CONTEXT, OBRIGAT, VLDUSER
			"","","",;							//CBOX, CBOX SPA, CBOX ENG
			"",'A210WhenMo(M->JAF_COD,M->JAF_VERSAO, "JHZ_CODMOD")',"","","","S"}) //PICTVAR, WHEN, INIBRW, SXG, FOLDER, PYME
...
aAdd(aSX3,{	"JHZ",;								//Arquivo
			...
			"Descricao",;					    //Titulo
			"Descricao",;					    //Titulo
			"Descricao do Modelo Curri",;		//Descricao
			"Descricao do Modelo Curri",;		//Descricao
			"Descricao do Modelo Curri",;		//Descricao
			"@!",;								//Picture
			"",;                            	//VALID
...
aAdd(aSX3,{	"JI2",;								//Arquivo
			...
			"Grupo de Atividade",;				//Descricao
			"@!",;								//Picture
			"A210VALGRP() .AND. ExistCpo('JHX',M->JI2_CODGRP)",; //VALID
			cUsadoObr,;							//USADO
			"",;								//RELACAO
			"JHX",;								//F3
			1,;									//NIVEL
...
aAdd(aSX3,{	"JI3",;								//Arquivo
			...
			"Tipo do componente",;				//Descricao
			"@!",;								//Picture
			"Pertence('12') .and. A110VlTpCom(M->JI3_TIPO)",;//VALID
			cUsadoObr,;							//USADO
			"",;								//RELACAO
			"",;								//F3
			1,;									//NIVEL
...
aAdd(aSX3,{	"JI3",;								//Arquivo
			...
			"",;		  						//VALID
			cUsadoObr,;							//USADO
			'Posicione("JHV",1,xFilial("JHV")+JI3->JI3_CODMOD,"JHV_DESC")',; //RELACAO
			"",;								//F3
			1,;									//NIVEL
			cReservObr,;						//RESERV
			"","","","N","V",;					//CHECK, TRIGGER, PROPRI, BROWSE, VISUAL
...
aAdd(aSX3,{	"JII",;								//Arquivo
			...
			"Codigo Grupo de Atividades",;		//Descricao
			"@!",;								//Picture
			'ACA675TpApt() .And. ExistCpo("JHX",M->JII_CODGRP) .And. ACA675VlAti()',;	//VALID
			cUsadoObr,;							//USADO
			'',;								//RELACAO
			"JHY",;								//F3
			1,;									//NIVEL
...
aAdd(aSX3,{	"JJJ",;								//Arquivo
			...
			"Rotina",;      			    	//Titulo ENG
			"Rotina",;	        				//Descricao
			"Rotina",;							//Descricao SPA
			"Rotina",;	 						//Descricao ENG
			"@!",;								//Picture
			"",;								//VALID
			cUsadoNao,;							//USADO
			"",;								//RELACAO
...
aAdd(aSX3,{	"JJJ",;								//Arquivo
			...
			"Tabela",;      			    	//Titulo ENG
			"Tabela",;	        				//Descricao
			"Tabela",;							//Descricao SPA
			"Tabela",;	 						//Descricao ENG
			"@!",;								//Picture
			"",;								//VALID
			cUsadoNao,;							//USADO
			"",;								//RELACAO
...
aAdd(aSX3,{	"JJJ",;								//Arquivo
			...
			0,;									//Decimal
			"Rec.Tabela",;     					//Titulo
			"Rec.Tabela",;	  					//Titulo SPA
			"Rec.Tabela",;     			    	//Titulo ENG
			"Recno da Tabela bloqueada",;		//Descricao
			"Recno da Tabela bloqueada",;		//Descricao SPA
			"Recno da Tabela bloqueada",;		//Descricao ENG
			"",;								//Picture
			"",;								//VALID
			cUsadoNao,;							//USADO
			"",;								//RELACAO
...
aAdd(aSX3,{	"JJJ",;								//Arquivo
			...
			8,;	     							//Tamanho
			0,;									//Decimal
			"Data bloqueio",;  					//Titulo
			"Data bloqueio",;			   		//Titulo SPA
			"Data bloqueio",; 			    	//Titulo ENG
			"Data do bloqueio",;				//Descricao
			"Data do bloqueio",;	   			//Descricao SPA
			"Data do bloqueio",;				//Descricao ENG
			"",;								//Picture
			"",;								//VALID
			cUsadoNao,;							//USADO
			"",;								//RELACAO
...
aAdd(aSX3,{	"JJJ",;								//Arquivo
			...
			8,;	     							//Tamanho
			0,;									//Decimal
			"Hora bloqueio",;  					//Titulo
			"Hora bloqueio",;			   		//Titulo SPA
			"Hora bloqueio",; 			    	//Titulo ENG
			"Hora do bloqueio",;				//Descricao
			"Hora do bloqueio",;	   			//Descricao SPA
			"Hora do bloqueio",;				//Descricao ENG
			"99:99",;							//Picture
			"",;								//VALID
			cUsadoNao,;							//USADO
			"",;								//RELACAO
...
SX3->( dbSetOrder(1) )
SX3->( dbSeek("JGM") )
while SX3->( !eof() .and. X3_ARQUIVO == "JGM" )
	i++
	SX3->( dbSkip() )
end
i++
aAdd(aSX3,{	"JGM",;								//Arquivo
			...
			"Disc. Compon",;				    //Titulo
			"Disc. Compon",;				    //Titulo
			"Disc. do Componente",; 			//Descricao
			"Disc. do Componente",;	   	    	//Descricao
			"Disc. do Componente",;	    		//Descricao
			"@!",;								//Picture
			"ExistCpo('JAE',M->JGM_DISCOM)",;	//VALID
			cUsadoOpc,;							//USADO
			"",;								//RELACAO
			"JAE",;								//F3
			1,;									//NIVEL
...
aAdd(aSX3,{	"JGM",;								//Arquivo
			...
			"Descricao do Componente",;			//Descricao
			"@!",;								//Picture
			"Posicione('JAE',1,xFilial('JAE')+JGM->JGM_DISCOM,'JAE_DESC')",; //VALID
			cUsadoOpc,;							//USADO
			"Iif(!Inclui,Posicione('JAE',1,xFilial('JAE')+JGM->JGM_DISCOM,'JAE_DESC'),'')",;//RELACAO
			"",;								//F3
			1,;									//NIVEL
			cReservOpc,;						//RESERV
			"","","","N","V",;					//CHECK, TRIGGER, PROPRI, BROWSE, VISUAL
...
aAdd(aSX3,{	"JGM",;								//Arquivo
			...
			"Cod. Modelo",;					    //Titulo
			"Cod. Modelo",;					    //Titulo
			"Cod. Modelo Curricular",;			//Descricao
			"Cod. Modelo Curricular",;			//Descricao
			"Cod. Modelo Curricular",;			//Descricao
			"@!",;								//Picture
			"ExistCpo('JHV',M->JGM_CODMOD) .and. A500VlMod(M->JGM_CODMOD)",; //VALID
			cUsadoObr,;							//USADO
			"",;								//RELACAO
			"JHV",;								//F3
			1,;									//NIVEL
...
aAdd(aSX3,{	"JGM",;								//Arquivo
			...
			"Descricao",;					    //Titulo
			"Descricao",;					    //Titulo
			"Descr. Modelo Curricular",;		//Descricao
			"Descr. Modelo Curricular",;		//Descricao
			"Descr. Modelo Curricular",;		//Descricao
			"@!",;								//Picture
			"",; 								//VALID
			cUsadoOpc,;							//USADO
			"Iif(!Inclui,Posicione('JHV',1,xFilial('JHV')+JGM->JGM_CODMOD,'JHV_DESC'),'')",;//RELACAO
			"",;								//F3
			1,;									//NIVEL
			cReservOpc,;						//RESERV
			"","","","N","V",;					//CHECK, TRIGGER, PROPRI, BROWSE, VISUAL
...
aAdd(aSX3,{	"JGM",;								//Arquivo
			...
			"Descricao",;					    //Titulo
			"Descricao",;					    //Titulo
			"Descr. Item do Modelo",;			//Descricao
			"Descr. Item do Modelo",;			//Descricao
			"Descr. Item do Modelo",;			//Descricao
			"@!",;								//Picture
			"",; 								//VALID
			cUsadoOpc,;							//USADO
			"Iif(!Inclui,Posicione('JHW',1,xFilial('JHW')+JGM->JGM_CODMOD+JGM->JGM_ITEMOD,'JHW_DESC'),'')",;//RELACAO
			"",;								//F3
			1,;									//NIVEL
			cReservOpc,;						//RESERV
			"","","","N","V",;					//CHECK, TRIGGER, PROPRI, BROWSE, VISUAL
...
//Altera o X3_VALID e X3_WHEN do campo JAY_CODDIS
if SX3->(dbSeek("JAY_CODDIS") ) .and. ( Alltrim(SX3->X3_VALID) <> 'A210ValGra() .and. A210VlDiAt("JAY_CODDIS")' .or. Alltrim(SX3->X3_WHEN) <> 'A210ValDis()' )
	RecLock("SX3",.F.)
	SX3->X3_VALID := 'A210ValGra() .and. A210VlDiAt("JAY_CODDIS")'
	SX3->X3_WHEN := 'A210ValDis()'
	SX3->( msUnlock() )
endif

//Altera o X3_VALID do campo JI2_CODGRP
if SX3->(dbSeek("JI2_CODGRP") ) .and. ( Alltrim(SX3->X3_VALID) <> 'A210VALGRP() .AND. ExistCpo("JHX",M->JI2_CODGRP) .and. A210VlDiAt("JI2_CODGRP")' )
	RecLock("SX3",.F.)
	SX3->X3_VALID := 'A210VALGRP() .AND. ExistCpo("JHX",M->JI2_CODGRP) .and. A210VlDiAt("JI2_CODGRP")'
	SX3->( msUnlock() )
endif
...
aAdd(aSX7,{	"JHZ_CODMOD",;			 		//Campo
			"001",;							//Sequencia
			"Posicione('JHV',1,xFilial('JHV')+M->JHZ_CODMOD,'JHV_DESC')",;		//Regra
			"JHZ_DESMOD",;      			//Campo Dominio
			"P",;              				//Tipo
			"N",;  							//Posiciona?
			"",;							//Alias
			0,;								//Ordem do Indice
...
aAdd(aSX7,{	"JGM_DISCOM",;			 		//Campo
			"001",;							//Sequencia
			"Posicione('JAE',1,xFilial('JAE')+M->JGM_DISCOM,'JAE_DESC')",;		//Regra
			"JGM_DESDIS",;      			//Campo Dominio
			"P",;              				//Tipo
			"N",;  							//Posiciona?
			"",;							//Alias
			0,;								//Ordem do Indice
...
aAdd(aSX7,{	"JGM_CODMOD",;			 		//Campo
			"001",;							//Sequencia
			"Posicione('JHV',1,xFilial('JHV')+M->JGM_CODMOD,'JHV_DESC')",;		//Regra
			"JGM_DESMOD",;      			//Campo Dominio
			"P",;              				//Tipo
			"N",;  							//Posiciona?
			"",;							//Alias
			0,;								//Ordem do Indice
...
//Exclui gatilho do campo JAS_CARGA
If SX7->( DbSeek("JAS_CARGA "+"001") )
	RecLock("SX7",.F.)
	SX7->( dbDelete() )
	SX7->( msUnlock() )	
EndIf
...
aAdd(aSIX,{	"JJJ",;   										//Indice
			"1",;                 							//Ordem
			"JJJ_FILIAL+JJJ_ROTINA+JJJ_TABELA+JJJ_RECTAB+JJJ_USER",;  					//Chave
			"Filial+Rotina+Tabela+Recno da tabela+Usuario",;     						//Descicao Port.
			"Filial+Rotina+Tabela+Recno da tabela+Usuario",;     						//Descicao Port.
			"Filial+Rotina+Tabela+Recno da tabela+Usuario",;     						//Descicao Port.
			"S",; 											//Proprietario
			"",;  											//F3
...
aAdd(aSXB,{	"JHV","4","01","01",;		// ALIAS, TIPO, SEQ, COLUNA
			...
			"Codigo",;					// DESCRI
			"Codigo",;					// DESCRI
			"JHV->JHV_CODMOD", "" })			// CONTEM, WCONTEM
aAdd(aSXB,{	"JHV","4","01","02",;		// ALIAS, TIPO, SEQ, COLUNA
			"Descricao",;				// DESCRI
			"Descricao",;				// DESCRI
			"Descricao",;				// DESCRI
			"JHV->JHV_DESC", "" })			// CONTEM, WCONTEM
...
aAdd(aSXB,{	"JHW","1","01","DB",;		// ALIAS, TIPO, SEQ, COLUNA
			"Itens Mod.Curricular",;	// DESCRI
			"Itens Mod.Curricular",;	// DESCRI
			"Itens Mod.Curricular",;	// DESCRI
			"JHW", "" })				// CONTEM, WCONTEM
aAdd(aSXB,{	"JHW","2","01","01",;		// ALIAS, TIPO, SEQ, COLUNA
			"Cod. Modelo+item",;		// DESCRI
			"Cod. Modelo+item",;		// DESCRI
			"Cod. Modelo+item",;		// DESCRI
			"", "" })					// CONTEM, WCONTEM
aAdd(aSXB,{	"JHW","4","01","01",;		// ALIAS, TIPO, SEQ, COLUNA
			"Item",;					// DESCRI
			"Item",;					// DESCRI
			"Item",;					// DESCRI
			"JHW_ITEMOD", "" })			// CONTEM, WCONTEM
aAdd(aSXB,{	"JHW","4","01","02",;		// ALIAS, TIPO, SEQ, COLUNA
			"Descricao",;				// DESCRI
			"Descricao",;				// DESCRI
			"Descricao",;				// DESCRI
			"JHW_DESC", "" })			// CONTEM, WCONTEM
aAdd(aSXB,{	"JHW","5","01","",;			// ALIAS, TIPO, SEQ, COLUNA
			"",;						// DESCRI
			"",;						// DESCRI
			"",;						// DESCRI
			"JHW->JHW_ITEMOD", "" })	// CONTEM, WCONTEM
aAdd(aSXB,{	"JHW","5","02","",;			// ALIAS, TIPO, SEQ, COLUNA
			"",;						// DESCRI
			"",;						// DESCRI
			"",;						// DESCRI
			"JHW->JHW_DESC", "" })		// CONTEM, WCONTEM
aAdd(aSXB,{	"JHW","6","01","",;			// ALIAS, TIPO, SEQ, COLUNA
			"",;						// DESCRI
			"",;						// DESCRI
			"",;						// DESCRI
			'JHW->JHW_CODMOD == GDFieldGet("JGM_CODMOD") .And. JHW->JHW_TPCONT == "1"', "" })	// CONTEM, WCONTEM
...
JHV->( dbSetOrder(1) ) //JHV_FILIAL+JHV_CODMOD - MODELOS CURRICULARES
...
If JHV-> ( !dbSeek( xFilial("JHV") + '000001' ) )
	...
	If JHW-> ( !dbSeek( xFilial("JHW") + '000001' + '001' ) )
		...
		JHW->JHW_DESC   := 'TEORIA'
		JHW->JHW_TPCONT := '1'
		JHW->JHW_CARGA  := '1' // não controla
		JHW->JHW_CREDIT := '1' // não controla
		MsUnLock()
	EndIf
	...
	If JHW-> ( !dbSeek( xFilial("JHW") + '000001' + '003' ) )
		RecLock("JHW",.T.) //Adiciona registro
		JHW->JHW_FILIAL := xFilial("JHW")
		JHW->JHW_CODMOD := '000001'
		JHW->JHW_ITEMOD := '003'
		JHW->JHW_DESC   := 'ATIVIDADES'
		JHW->JHW_TPCONT := '2'
		JHW->JHW_CARGA  := '1'
		JHW->JHW_CREDIT := '2'
		MsUnLock()
...
While !trbJAF->(Eof())	.and. lOk  		
	...
	If !trbJAF->JAF_COD+trbJAF->JAF_VERSAO == cCURPAD
		...
		AcaLog( cArquivo, dtoc( Date() )+" "+Time()+" "+"Atualizando curso " + trbJAF->JAF_COD+" "+trbJAF->JAF_VERSAO+" Filial "+xFilial("JAF") )
	...
	Else
		...
		AcaLog( cErros, dtoc( Date() )+" "+Time()+" "+"Disciplina nao Localizada: " + trbJAF->JAY_CODDIS + ". Curso padrao: " + trbJAF->JAF_COD + " Filial: " + xFilial("JAE") )
	EndIf
...
While !trbJAH->(Eof())	.and. lOk  		
	...
	If !trbJAH->JAH_CODIGO == cCURVIG
		...
		AcaLog( cArquivo, dtoc( Date() )+" "+Time()+" "+"Atualizando curso vigente " + trbJAH->JAH_CODIGO + " Filial "+xFilial("JAH") )
	...
	Else
		...
		AcaLog( cErros, dtoc( Date() )+" "+Time()+" "+"Disciplina nao Localizada: " + trbJAH->JAS_CODDIS + ". Curso vigente: " + trbJAH->JAH_CODIGO + " Filial: " + xFilial("JAE") )
	EndIf
...
If	JAY->( DbSeek( xFilial("JAY")+trbJAF->JAF_COD+trbJAF->JAF_VERSAO+trbJAF->JAY_PERLET+trbJAF->JAY_HABILI+trbJAF->JAY_CODDIS) )			
...
if JHZ->( !dbSeek( xFilial("JHZ")+cCurso+cVersao+trbJAH->JAS_PERLET+trbJAH->JAS_HABILI+cDisJI3 ) )
	...
	AcaLog( cErros, dtoc( Date() )+" "+Time()+" "+"Sem componente - Não foi possivel encontrar o componente para a disciplina " + cDisJI3 +" do curso vigente "+trbJAH->JAS_CODCUR+" período "+trbJAH->JAS_PERLET+" filial "+xFilial("JHZ") )	
	Return .T.
endif
...
If JAS->( DbSeek( xFilial("JAS")+trbJAH->JAS_CODCUR+trbJAH->JAS_PERLET+trbJAH->JAS_HABILI+trbJAH->JAS_ITEM) )
...
while lQryJA2
	...
	cQuery := "select DISTINCT JC7_NUMRA, JC7_CODCUR, JI3_COMP "
	cQuery += "  from  "
	cQuery += RetSQLName("JC7")+" JC7, "
	cQuery += RetSQLName("JAS")+" JAS, "
	cQuery += RetSQLName("JI3")+" JI3, "
	cQuery += RetSQLName("JA2")+" JA2  "
	cQuery += " where JC7_FILIAL = '"+xFilial("JC7")+"' "
	cQuery += "   and JAS_FILIAL = '"+xFilial("JAS")+"' "
	cQuery += "   and JI3_FILIAL = '"+xFilial("JI3")+"' "
	cQuery += "   and JA2_FILIAL = '"+xFilial("JA2")+"' "
	cQuery += "   and JC7_CODCUR = JAS_CODCUR "
	cQuery += "   and JAS_CODCUR = JI3_CODCUR "
	cQuery += "   and JAS_PERLET = JI3_PERLET "
	cQuery += "   and JAS_HABILI = JI3_HABILI "
	cQuery += "   and JAS_COMP   = JI3_COMP "
	cQuery += "   and JA2_NUMRA  = JC7_NUMRA "
	cQuery += "   and JC7_SITUAC not in ('7','8','9','A') " //7=Trancado;8=Dispensado;9=Cancelado;A=A Cursar
	cQuery += "   and JA2.R_E_C_N_O_ between '" + Str(nREC_I) + "' AND '" + Str(nREC_F) + "'"
	cQuery += "   and JC7.D_E_L_E_T_ = ' ' "
	cQuery += "   and JAS.D_E_L_E_T_ = ' ' "
	cQuery += "   and JI3.D_E_L_E_T_ = ' ' "
	cQuery += "   and JA2.D_E_L_E_T_ = ' ' "
	cQuery += " order by JC7_NUMRA, JC7_CODCUR, JI3_COMP "
	...
	cQueryJob	:= " SELECT DISTINCT JC7_NUMRA, JC7_CODCUR, JC7_PERLET, JC7_HABILI, JC7_TURMA, JC7_OUTCUR, JC7_OUTPER, JC7_OUTHAB, JC7_OUTTUR, JI3_COMP "
	cQueryJob	+= "    FROM "
	cQueryJob	+= RetSQLName("JC7")+" JC7, "
	cQueryJob	+= RetSQLName("JAS")+" JAS, "
	cQueryJob	+= RetSQLName("JI3")+" JI3, "
	cQueryJob	+= RetSQLName("JA2")+" JA2  "
	cQueryJob	+= "   where JC7_FILIAL = '"+xFilial("JC7")+"' "
	cQueryJob	+= "     and JAS_FILIAL = '"+xFilial("JAS")+"' "
	cQueryJob	+= "     and JI3_FILIAL = '"+xFilial("JI3")+"' "
	cQueryJob	+= "     and JA2_FILIAL = '"+xFilial("JA2")+"' "
	cQueryJob	+= "     and JAS_CODCUR = JC7_CODCUR " 
	cQueryJob	+= "     and JAS_PERLET = JC7_PERLET "
	cQueryJob	+= "     and JAS_HABILI = JC7_HABILI "
	cQueryJob	+= "     and JAS_CODDIS = JC7_DISCIP "
	cQueryJob	+= "     and JAS_CODCUR = JI3_CODCUR "
	cQueryJob	+= "     and JAS_PERLET = JI3_PERLET "
	cQueryJob	+= "     and JAS_HABILI = JI3_HABILI "
	cQueryJob	+= "     and JAS_COMP   = JI3_COMP   "
	cQueryJob	+= "     and JA2_NUMRA  = JC7_NUMRA  "
	cQueryJob	+= "     and JC7_SITUAC not in ('7','8','9','A') "
	cQueryJob   += "     and JA2.R_E_C_N_O_ between '" + Str(nREC_I) + "' AND '" + Str(nREC_F) + "'"
	cQueryJob   += "     and JC7.D_E_L_E_T_ = ' ' "
	cQueryJob   += "	 and JAS.D_E_L_E_T_ = ' ' "
	cQueryJob   += "	 and JI3.D_E_L_E_T_ = ' ' "
	cQueryJob   += "	 and JA2.D_E_L_E_T_ = ' ' "
	cQueryJob	+= "  ORDER BY JC7_NUMRA, JC7_CODCUR, JC7_PERLET, JC7_HABILI, JC7_TURMA, JC7_OUTCUR "
	...
	if lQryJA2
		...
		while (cAlias)->( !eof() )
			...
			if JI6->( !dbSeek( xFilial("JI6")+(cAlias)->(JC7_NUMRA+JC7_CODCUR+JI3_COMP) ) )
	
				RecLock("JI6",.T.)
				JI6->JI6_FILIAL	:= xFilial("JI6")
				JI6->JI6_NUMRA	:= (cAlias)->JC7_NUMRA
				JI6->JI6_CODCUR	:= (cAlias)->JC7_CODCUR
				JI6->JI6_COMP	:= (cAlias)->JI3_COMP
				JI6->JI6_SITUAC	:= "A"
				JI6->( msUnlock() )
								
				AcaLogTab( cArquivo, "IncluiJI6", "RA: " + JI6->JI6_NUMRA, "" )
...
while (cAlias)->( !eof() )
	...
	if JI6->( dbSeek( xFilial("JI6") + (cAlias)->(JC7_NUMRA + JC7_CODCUR + JI3_COMP) ) )
		...
		JAS->( dbSetOrder(7) ) //JAS_FILIAL+JAS_CODCUR+JAS_PERLET+JAS_COMP+JAS_CODDIS
		if JAS->( dbSeek( xFilial("JAS") + (cAlias)->(JC7_CODCUR + JC7_PERLET + JI3_COMP) ) )
			
			//Executa a funcao ACMedPai apenas uma vez para cada Aluno
			if (cAlias)->JC7_NUMRA <> cRaAnt
				AcaLogTab( cArqLote, "JobMedia", "* Inicio  ACMedPai *", JI6->JI6_NUMRA)
				ACMedPai( JI6->JI6_NUMRA, (cAlias)->JC7_CODCUR, (cAlias)->JC7_PERLET, (cAlias)->JC7_HABILI, (cAlias)->JC7_TURMA )
				AcaLogTab( cArqLote, "JobMedia", "*** Fim ACMedPai ***", JI6->JI6_NUMRA)
			
				cRaAnt := (cAlias)->JC7_NUMRA
			endif
			
			JC7->( dbSetOrder(4) ) //JC7_FILIAL+JC7_NUMRA+JC7_CODCUR+JC7_DISCIP
			If JC7->( dbSeek( xFilial("JC7")+ (cAlias)->(JC7_NUMRA)+JAS->JAS_CODCUR+JAS->JAS_CODDIS) )	
				JI6->( dbSeek( xFilial("JI6") + (cAlias)->(JC7_NUMRA + JC7_CODCUR + JI3_COMP) ) )
				RecLock("JI6",.F.)
				JI6->JI6_SITUAC := iif( JC7->JC7_SITUAC == "6", "1", iif( JC7->JC7_SITUAC $ "7|8|9", JC7->JC7_SITUAC, JI6->JI6_SITUAC ) ) //7=Trancado;8=Dispensado;9=Cancelado
				JI6->( Msunlock() )
			Endif                  
		Else
			AcaLog( "UpdErr.LOG", dtoc( Date() )+" "+Time()+" "+"JobMedia - Não encontrou JAS - Emp. "+cEmp+" Fil. "+xFilial("JAS")+ " Cur. Vig. "+(cAlias)->JC7_CODCUR+" per.let. " +(cAlias)->JC7_PERLET +" Componente " + (cAlias)->JI3_COMP )	
		Endif                      
	else
		AcaLog( "UpdErr.LOG", dtoc( Date() )+" "+Time()+" "+"JobMedia - Não encontrou JI6 - Emp. "+cEmp+" Fil. "+xFilial("JI6") +" RA: "+(cAlias)->JC7_NUMRA+" Cur.Vig. "+(cAlias)->JC7_CODCUR +" Componente " +(cAlias)->JI3_COMP )	
	endif
