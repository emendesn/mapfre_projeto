...
EndIf
If lSubTurma
	uRet := ExecBlock( "SEC0020e", .F., .F., { lWeb } )
Endif
...
If aRet[ 11 ] == "2"		//	Permuta
	lRet := ACTransfere( 1, 3, 4, 6, 1, 3, 4, 10, .F.,26, 12 )
EndIf
...
If lRet
	...
	//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ*/
		...
		If cTipGrd == "1" //Grade Semanal
			cQuery := "SELECT DISTINCT JBL_SUBTUR SUBTUR FROM " + RetSQLName("JBL")
		...
		Else //Diaria
			cQuery := "SELECT DISTINCT JD2_SUBTUR SUBTUR FROM " + RetSQLName("JD2")
		...
		If !empty(M->JBH_SCP14)			
			...
			Endif
		Else
			While _QRYSUBTUR->(!Eof())
				If M->JBH_SCP14 <> _QRYSUBTUR->SUBTUR
					lRet := .F.
					If !lWeb
						MsgStop( "Grade de aulas com sub-turma. Favor informar a sub-turma." )
					Else
						aadd(aRet,{.F.,"Grade de aulas com sub-turma. Favor informar a sub-turma."})
		        		Return aRet
					Endif
					Exit
				EndIf
