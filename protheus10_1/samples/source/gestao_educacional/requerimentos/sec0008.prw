...
// Verifica se Aluno compareceu na Avaliação Dissertativa para gerar Requerimento	
...
if ! lRet
	...
	if cTpDis <> JBQ->JBQ_TIPO
		
		lRet := .F.
		
		if lWeb
			aadd( aRet, {.F., "A avaliação "+AllTrim(JBQ->JBQ_DESC)+" não é uma avaliação do tipo "+AcaX3Combo("JBQ_TIPO",cTpDis)+"."} )
		else
			MsgStop( "A avaliação "+AllTrim(JBQ->JBQ_DESC)+" não é uma avaliação do tipo "+AcaX3Combo("JBQ_TIPO",cTpDis)+"." )
		endif
...
if len(aRet) == 0 // continua validacao
	...
	if len(aRet) == 0 // continua a validacao com o regime
		...
		if nPos > 0
			if !Empty(JC7->JC7_SITDIS)
				cRegime := JC7->JC7_SITDIS
			else
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPrograma  ³ SEC0008G º Autor ³ Alberto Deviciente º Data ³ 23/Mai/07   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDescricao ³ Busca a situacao da disciplina do aluno                    º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºUso       ³ Requerimento de Avaliacao Substitutiva                     º±±
±±ÈÍÍÍÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¼±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
*/
User Function SEC0008G()
Local aArea 	:= GetArea()
Local cNumRa 	:= PadR(M->JBH_CODIDE, TamSX3("JC7_NUMRA")[1])
Local cCodCur 	:= PadR(M->JBH_SCP01, TamSX3("JC7_CODCUR")[1]) //Cod. Cur. Vig.
Local cPerLet 	:= PadR(M->JBH_SCP03, TamSX3("JC7_PERLET")[1]) //Per. letivo
Local cHabili 	:= PadR(M->JBH_SCP04, TamSX3("JC7_HABILI")[1]) //Habilitacao
Local cTurma  	:= PadR(M->JBH_SCP06, TamSX3("JC7_TURMA")[1])  //Turma
Local cDiscip 	:= PadR(M->JBH_SCP11, TamSX3("JC7_DISCIP")[1])  //Disciplina
Local aRet

JC7->( dbSetOrder(1) ) //JC7_FILIAL+JC7_NUMRA+JC7_CODCUR+JC7_PERLET+JC7_HABILI+JC7_TURMA+JC7_DISCIP+...
JC7->( DbSeek( xFilial("JC7")+cNumRa+cCodCur+cPerLet+cHabili+cTurma+cDiscip ) )

M->JBH_SCP13 := JC7->JC7_SITDIS //Regime
M->JBH_SCP14 := Tabela("F3", JC7->JC7_SITDIS, .F.) //Descr. Regime

if Empty(M->JBH_SCP13)
	aRet := .F.
else
	aRet := {{"SCRIPT"}} //Indica que ira atualizar os campos do Script do Requerimento
endif
