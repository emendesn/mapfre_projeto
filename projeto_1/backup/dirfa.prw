     ...
     If SE2->(DbSeek(_cchavet))
        ...
        IF ( SE2->E2_NATUREZ == PADR("IRF",10," ")  ) .AND. ( SE2->E2_TIPO == "TX " )
            ...
            If SubStr(cCombo,1,1)="2"
               ...
               IF cPessoa = "J"
                  REPLACE SE2->E2_CODRET WITH "1708" 
               ELSE
                  REPLACE SE2->E2_CODRET WITH "0561" 
               ENDIF           
               SE2->(MSUNLOCK())   
        ...
        If ( SE2->E2_TIPO == "TX " )  .AND.   ;
           ...
           If SubStr(cCombo,1,1)="2"  
              RECLOCK("SE2", .F. )
              REPLACE SE2->E2_DIRF WITH "1"
              REPLACE SE2->E2_CODRET WITH "5952" 
              SE2->(MSUNLOCK())    
              If !TMPSE2->(DbSeek(SE2->E2_FILIAL+SE2->E2_PREFIXO+SE2->E2_NUM+SE2->E2_PARCELA+SE2->E2_TIPO+SE2->E2_FORNECE+SE2->E2_LOJA))
                 RECLOCK("TMPSE2", .T. )
                 TMPSE2->E2_FILIAL  :=SE2->E2_FILIAL
...
If nOpca = 1
   ...
   If SE2->(DbSeek(_cchavet))           
      RECLOCK("SE2", .F. )
      REPLACE SE2->E2_DIRF   WITH SubStr(cCombo,1,1)
      REPLACE SE2->E2_CODRET WITH _ccodret
      SE2->(MSUNLOCK())   
      If !TMPSE2->(DbSeek(SE2->E2_FILIAL+SE2->E2_PREFIXO+SE2->E2_NUM+SE2->E2_PARCELA+SE2->E2_TIPO+SE2->E2_FORNECE+SE2->E2_LOJA)) 
         RECLOCK("TMPSE2", .T. )
         TMPSE2->E2_FILIAL  :=SE2->E2_FILIAL
