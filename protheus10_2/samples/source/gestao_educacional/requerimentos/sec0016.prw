...
JAY->( dbSeek( xFilial("JAY")+cCurso+cVersao, .T. ) )

while !JAY->( eof() ) .and. JAY->JAY_FILIAL+JAY->JAY_CURSO+JAY->JAY_VERSAO == xFilial("JAY")+cCurso+cVersao
...
Local lMV_ACSTAG := GetNewPar('MV_ACSTAG',.F.)

Local lJaExc 	:= Getnewpar('MV_ACBLQAN',.F.) // SE FOR .T. Nao realiza a reserva de vaga
Local lSubTur   := JBE->( FieldPos( "JBE_SUBTUR" ) ) > 0
...
while !JBO->( eof() ) .and. JBO->JBO_FILIAL+JBO->JBO_CODCUR+JBO->JBO_PERLET+JBO->JBO_HABILI== xFilial("JBO")+JCS->JCS_CURSO+JCS->JCS_SERIE+JCS->JCS_HABILI
	...
	if AcaVerJBO(JAR->JAR_CODCUR, JAR->JAR_PERLET, JAR->JAR_HABILI, JBO->JBO_TURMA, 4)
		JBK->( dbSetOrder(1) )
		If JBK->( dbSeek(xFilial("JBK") + JBO->JBO_CODCUR+JBO->JBO_PERLET+JBO->JBO_HABILI+JBO->JBO_TURMA ) )
			lVaga := .T.
			exit
		Else // Se existir na JBO e nใo existir na JBK 
		    lGradevaga:= .F. 
			exit
		EndIf 
	endif
...
if !lVaga .and. lGradevaga
	MsgStop("Nใo existe vaga disponํvel em nenhuma turma deste curso.")
	Return .F.
elseif !lvaga .and. !lGradevaga           
	MsgStop("Existem vagas para este curso, por้m nใo existe grade de aulas cadastrada para as turmas disponํveis.")
	Return .F.
endif
...
For nA := 1 To Val(JCS->JCS_SERIE)
	...
	if !lJaExc 
		...
		elseif !JBE->( dbSeek( xFilial("JBE")+cNumRA+JCS->JCS_CURSO+cSerie+JCS->JCS_HABILI ) )
			...
			cTurma	:= if( nA == Val(JCS->JCS_SERIE), JBO->JBO_TURMA, GetTurma(JCS->JCS_CURSO,cSerie,JCS->JCS_HABILI,JBO->JBO_TURMA) )
			...
			If lSubTur .and. JCS->( FieldPos( "JCS_SUBTUR" ) ) > 0
		...
		if JBE->( dbSeek( xFilial("JBE")+cNumRA+JCS->JCS_CURSO+cSerie+JCS->JCS_HABILI ) ) .and.	Val(JCS->JCS_SERIE) > nA .AND. JBE->JBE_ATIVO = '3'
			//coloca como ativo = Nao os periodos anteriores 
			RECLOCK('JBE',.F.)
			JBE->JBE_ATIVO = '2'
			MSUNLOCK()
		elseif !JBE->( dbSeek( xFilial("JBE")+cNumRA+JCS->JCS_CURSO+cSerie+JCS->JCS_HABILI ) )
	...
	While !JCT->( eof() ) .and. JCT->JCT_NUMREQ == JCS->JCS_NUMREQ .and. JCT->JCT_PERLET == cSerie .and. JCT->JCT_HABILI == JCS->JCS_HABILI
		...
		// faz a verifica็ใo da situa็ใo da disciplina conforme o curso vigente destino.
		IF JCT->JCT_SITUAC == "003"
			cSituacao := '8'
		ELSEIF JCT->JCT_SITUAC $ "001/002"
			cSituacao := "A"
		ELSEIF JCT->JCT_SITUAC == "010" .AND. JCT->JCT_MEDFIM < JAR->JAR_MEDAPR .and. JCT->JCT_PERLET < JCS->JCS_SERIE
			cSituacao := "3"
		ELSEIF JCT->JCT_SITUAC == "010" .AND. JCT->JCT_MEDFIM >= JAR->JAR_MEDAPR 
			cSituacao := "2"
		ELSEIF JCT->JCT_SITUAC == "006" .AND. JCT->JCT_MEDFIM < JAR->JAR_MEDAPR .and. JCT->JCT_PERLET < JCS->JCS_SERIE
			cSituacao := "3"
		ELSEIF JCT->JCT_SITUAC == "006" .AND. JCT->JCT_MEDFIM >= JAR->JAR_MEDAPR
			cSituacao := "2"
		ELSE
			cSituacao := "1"
		ENDIF
		...
		If JCT->JCT_SITUAC $ "010;002;006" .and. Val(JCT->JCT_PERLET) < Val(JCS->JCS_SERIE)
			...
			Do While JC7->( ! EoF() .and. JC7_NUMRA + JC7_DISCIP == cNumRA + cDisEqv )
				If !lMV_ACSTAG // pega status da disciplina ja cursada anteriormente
		...
		If nRecno > 0 .And. GetNewPar( "MV_ACDPMX", .F. ) == .F.
			JCT->(dbGoto(nRecno))
			JC7->(dbSetOrder(1))
			If !(JC7->(dbSeek(xFilial('JC7')+cNumRa+aRet[1]+cSerie+JCT->JCT_HABILI+aRet[6]+JCT->JCT_DISCIP)))
			  	Aviso('Aten็ใo','A media final JCT_MEDFIN nao pode ser alterado.',{'Ok'})
			EndIF
			JCT->(RecLock("JCT",.F.))		
		...
		While JBL->( !eof() .and. JBL_FILIAL+JBL_CODCUR+JBL_PERLET+JBL_HABILI+JBL_TURMA+JBL_CODDIS == xFilial("JBL")+JCS->JCS_CURSO+JCT->JCT_PERLET+JCT->JCT_HABILI+JBE->JBE_TURMA+JCT->JCT_DISCIP )
			If lSubTur .and. JCS->( FieldPos( "JCS_SUBTUR" ) ) > 0
				If !Empty(JBL->JBL_SUBTUR) .and. JBL->JBL_SUBTUR <> JCS->JCS_SUBTUR
					JBL->(DBSkip())
					Loop
				EndIf
			Endif
			...
			JC7->JC7_PERLET := JCT->JCT_PERLET
			JC7->JC7_HABILI := JCT->JCT_HABILI
...
//Atualiza as vagas
AcaVerJBO(JCS->JCS_CURSO, JCS->JCS_SERIE, JCS->JCS_HABILI, cTurma, 3)
	...
	if JBE->( dbSeek( xFilial( "JBE" ) + cNumRA + aRet[1] ) )
		...
		if JBE->( dbSeek( xFilial( "JBE" ) + cNumRA + aRet[1] + cPerAtu ) )
			...
			If lSubTur .and. JCS->( FieldPos( "JCS_SUBTUR" ) ) > 0
				JBE->JBE_SUBTUR := JCS->JCS_SUBTUR		
			Endif
			JBE->JBE_ATIVO  := if( JBE->JBE_SITUAC == "1", "2", "1" )
			JBE->(MsUnlock())
		endif
...
//ณSe existir componentes e for de retorno de outro curso, gera a JI6  ณ
//ณdo aluno                                                            ณ
//ภฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤู

If lExistJI6
	//ฺฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฟ
	//ณVarre a tabela JI3 buscando todos os componentes do curso de destinoณ
	//ณcria o compoenente do aluno na JI6 do curso de destino              ณ
	//ภฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤู
	JI6->(DBSetOrder(1)) //JI6_FILIAL+JI6_NUMRA+JI6_CODCUR+JI6_COMP
				
	dbSelectArea("JI3")
	JI3->( dbSetOrder(2) ) //ORDEM: JI3_FILIAL+JI3_CODCUR+JI3_COMP
	JI3->(dbSeek(xFilial("JI3") + JCS->JCS_CURSO))
					
	While !JI3->(Eof()) .And. JI3->JI3_FILIAL+JI3->JI3_CODCUR == xFilial("JI3")+JCS->JCS_CURSO
					
		If JI6->(!DBSeek(xFilial("JI6") + cNumRA + JCS->JCS_CURSO + JI3->JI3_COMP))	
			RecLock("JI6", .T.)      
		Else                   
			RecLock("JI6", .F.)
		EndIf	
		JI6->JI6_FILIAL := xFilial("JI6")
		JI6->JI6_NUMRA  := cNumRA
		JI6->JI6_CODCUR := JCS->JCS_CURSO
		JI6->JI6_COMP	:= JI3->JI3_COMP
		If JI3->JI3_FILIAL+JI3->JI3_CODCUR+JI3->JI3_PERLET+JI3->JI3_HABILI == xFilial("JI3")+JCS->JCS_CURSO+JCS->JCS_SERIE+JCS->JCS_HABILI
			JI6->JI6_SITUAC := "1" 	//1-Cursando
			JI6->JI6_ANOLET := Posicione( "JAR", 1, xFilial("JAR") + JCS->JCS_CURSO + JCS->JCS_SERIE + JCS->JCS_HABILI, "JAR_ANOLET" )
			JI6->JI6_PERIOD := Posicione( "JAR", 1, xFilial("JAR") + JCS->JCS_CURSO + JCS->JCS_SERIE + JCS->JCS_HABILI, "JAR_PERIOD" )
		Else
			JI6->JI6_SITUAC := "A" 	//A-A Cursar						
		EndIf	
		JI6->(MsUnLock())
	...
	//ณ Atualiza situa็ใo no componente curricular do curso destino  dos proximos periodos
	//ภฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤู
	ACCancComp(cNumRA, JI3->JI3_CODCUR, "A")                     
	
	JI3->(dbSeek(xFilial("JI3") + JCS->JCS_CURSO))					
	While !JI3->(Eof()) .And. JI3->JI3_FILIAL+JI3->JI3_CODCUR == xFilial("JI3")+JCS->JCS_CURSO
		//ฺฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฟ
		//ณ Altera situa็ใo no componente curricular do curso destino 
		//ภฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤู
		ACA060AtJI6(JI3->JI3_CODCUR , JI3->JI3_PERLET , JI3->JI3_HABILI,cNumRA )				
		JI3->(DbSkip())
	EndDo
...
ฑฑ			  cKey: chave de busca na JBE					              บฑฑ
ฑฑ            lWeb: .T. - chamado pela Web								  บฑฑ
ฑฑฬออออออออออุออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออนฑฑ
...
ฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑ
ฑฑษออออออออออัออออออออออหอออออออัออออออออออออออออออออหออออออัอออออออออออออปฑฑ
ฑฑบPrograma  ณGetTurma  บAutor  ณRafael Rodrigues    บ Data ณ 03/Out/2006 บฑฑ
ฑฑฬออออออออออุออออออออออสอออออออฯออออออออออออออออออออสออออออฯอออออออออออออนฑฑ
ฑฑบDesc.     ณ Busca uma turma valida para o aluno no periodo letivo e    บฑฑ
ฑฑบ          ณ habilitacao definidos nos parametros.                      บฑฑ
ฑฑฬออออออออออุออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออนฑฑ
ฑฑบUso       ณ SEC0016b                                                  บฑฑ
ฑฑศออออออออออฯออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออผฑฑ
ฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑ
฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿
*/
