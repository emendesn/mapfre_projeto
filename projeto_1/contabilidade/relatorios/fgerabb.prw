...
±±ÚÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄ¿±±
±±³Programa  |fgeraBB     ³Ajustes para geração de todos os blocos ³ Arnaldo F de Souza #AFS |Data ³04/02/2015³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
ÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜ*/
...
Static Procedure Resultado(lEnd, oProcess, cLocal,dDataIni,dDataFim,cPlanRef,cCombo)
...
Private nHand1
	...
	//³ Seleciona os registros no banco de dados.               ³
	//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
	nTamTotal    := TamSX3("CT2_VALOR")[1]
	nTamDecimais := TamSX3("CT2_VALOR")[2]
	
	If ! TRB->( Eof()  )
		
		//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
		//³ Cria o arquivo temporario totalizando as contas selecionadas.³
		//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
		cArq_TMP := CriaTrab( {{ "CONTA"   , "C", 30, 0 },;
		                        { "TOTAL"   , "N", nTamTotal, nTamDecimais },;
		                        { "SINAL"   , "C", 1, 0 }}, .T.)
		DBUseArea( .T.,, cArq_TMP, "TMP", Nil, .F. )
		
		DBSelectArea("TMP")
		cArq_IND := GetNextAlias()
		IndRegua( "TMP", cArq_IND, "FIELD->CONTA",,, "Criando indice temporario...")
		
		// ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
		// ³Agrupa e totaliza as contas.³
		// ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
		While TRB->( .Not. Eof() )
			
			// ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
			// ³Regra do sinal para ser gravado no arquivo.³
			// ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
			If !Empty(TRB->CT2_DEBITO)
				cNormal	:= Posicione("CT1",1,xFilial("CT1")+Rtrim(TRB->CT2_DEBITO),"CT1_NORMAL")
			Else
				cNormal	:= Posicione("CT1",1,xFilial("CT1")+Rtrim(TRB->CT2_CREDIT),"CT1_NORMAL")
			EndIf
			cSinal := IIf(cNormal=="1",IIf(Left(TRB->CVD_CTAREF,1)$"123","+","-"),IIf(Left(TRB->CVD_CTAREF,1)$"123","-","+"))
			
			// ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
			// ³Grava os dados na tabela temporaria.       ³
			// ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
			IF .Not. TMP->( DBSeek( TRB->CVD_CTAREF ) )
				RecLock( "TMP", .T. )
				TMP->CONTA := TRB->CVD_CTAREF
				TMP->TOTAL := TRB->TOTAL
				TMP->SINAL := cSinal
				MSUnLock()
			Else
				RecLock( "TMP", .F. )
				TMP->TOTAL += NoRound( TRB->TOTAL, nTamDecimais )
				MSUnLock()
			Endif
		...
		oProcess:SetRegua2(TMP->(LastRec()))
		
		// ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
		// ³Cria o arquivo do CADDOC. ³
		// ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
		IF ( nHand1 := FCREATE( cLocal, FC_NORMAL ) ) > 0
			...
			TMP->( DBGoTop() )
			While TMP->( ! Eof() ) .And. nErro == 0
				
				oProcess:IncRegua2("Processando : " + TMP->CONTA +" Valor : "+ AllTrim(Transform(TMP->TOTAL,"@E 9,999,999.99"))+" ")
				
				// ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
				// ³Codigo da conta, removendo a formatacao.³
				...
				// ³Informacoes referente a primeira coluna de valor.³
				// ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
				//   fWrite(nHand1,TRB->CVD_CTAREF )
				fWrite(nHand1, STRZERO( VAL( cString ), 10 ) )
				fWrite(nHand1,Space(04))
				//				fWrite(nHand1,STRZERO( TMP->TOTAL, 18 ) )
				cInt := ALLTRIM( LEFT( STR( TMP->TOTAL ), AT(".",  STR( TMP->TOTAL ) ) -1 ) )
				cDec := PADR( RIGHT( STR( TMP->TOTAL ), LEN( STR( TMP->TOTAL) ) - RAT(".",  STR( TMP->TOTAL) ) ), 2, "0" )
				fWrite(nHand1, PADL( cInt + cDec, 18, "0" ) )      // Adicionado em 04/02/2014 #AFS
				
				// fWrite(nHand1,Space(18)) // comentado em 04/02/ 2014 #AFS
				
				// ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
				// ³Regra de sinais.                                 ³
				// ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
				fWrite(nHand1, TMP->SINAL )
				
				// ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
				// ³Informacoes referente a segunda coluna de valor.³
				// ³onde o valor da coluna eh igual a da primeira   ³
				// ³somente para as contas 1 e 4.                   ³
				// ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
				IF LEFT( TMP->CONTA, 1 ) $ "14"
					cInt := ALLTRIM( LEFT( STR( TMP->TOTAL ), AT(".",  STR( TMP->TOTAL ) ) -1 ) )
					cDec := PADR( RIGHT( STR( TMP->TOTAL ), LEN( STR( TMP->TOTAL) ) - RAT(".",  STR( TMP->TOTAL) ) ), 2, "0" )
					fWrite(nHand1, PADL( cInt + cDec, 18, "0" ) )      // Adicionado em 04/02/2014 #AFS
				ELSE
					fWrite(nHand1,STRZERO( 0, 18 ) )
				ENDIF
				//fWrite(nHand1,Space(18))  comentado em 04/02/ 2014 #AFS
				
				// ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
				// ³Grava  o sinal do registro.                      ³
				// ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
				fWrite(nHand1, TMP->SINAL )
				
				// ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
				// ³Informacoes referente a terceira coluna de valor.³
				// ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
				fWrite(nHand1,STRZERO( 0, 18 ) )
			...
			fWrite(nHand1,"@1")
			fWrite(nHand1,STRZERO( VAL( cLinha ), 6 ) )
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß*/
Static Procedure fMontaQuery(dDataIni,dDataFim,cPlanRef) 
...
Local cAlias := "TRB"
	...
	cQuery := "SELECT CT2.CT2_DEBITO,CT2.CT2_CREDIT,SUM(CT2.CT2_VALOR) TOTAL,CVD.CVD_CONTA,CVD.CVD_CTAREF,CT1_DESC01,CVN_DSCCTA"
	cQuery += " FROM " + RetSqlName("CT1")+ " CT1"
	
	cQuery += " INNER JOIN  " + RetSqlName("CVD")+ " CVD"
	cQuery += "       ON CVD.D_E_L_E_T_ = ' ' AND CVD_FILIAL = '"+xFilial("CVD")+"' AND CVD_CONTA = CT1.CT1_CONTA AND CVD.CVD_CODPLA = '"+cPlanRef+"'"
	
	cQuery += " INNER JOIN " + RetSqlName("CT2")+ " CT2"
	cQuery += "       ON CT2.D_E_L_E_T_= ' '"
	
	cQuery += " INNER JOIN " + RetSqlName("CVN")+ " CVN"
	cQuery += "       ON CVN.D_E_L_E_T_= ' ' AND CVN_CTAREF = CVD_CTAREF"
	
	cQuery += " WHERE CT1.D_E_L_E_T_ = ' ' AND CT1_FILIAL = '"+xFilial("CT1")+"' AND CT1.CT1_CONTA = CVD.CVD_CONTA AND CT2.CT2_DEBITO = CT1.CT1_CONTA"
	cQuery += "       AND CT2.CT2_DATA  BETWEEN '" + dDataIni + "' AND '" + dDataFim + "'"
	
	cQuery += " GROUP BY CT2_DEBITO,CT2_CREDIT,CVD_CONTA,CVD_CTAREF,CT1_DESC01,CVN_DSCCTA "
