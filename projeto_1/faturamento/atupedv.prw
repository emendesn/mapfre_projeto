//#INCLUDE "TOPCONN.CH"	//#JN20141222.o
#Include "Protheus.Ch"
//#Include "FONT.CH"	//#JN20141222.o
//#include "rwmake.ch"	//#JN20141222.o
...
±±³Jose Novaes ³ 22/12/2014 ³ #JN20141222 - Atualizacoes no fonte e       ³±±
±±³            ³            ³ gravacao do campo C6_CODISS.                ³±±
...
Local aCampos		:= {}
Local aCores        := {}
//Local aCampos1		:= {}	//#JN20141222.o
Local oMark	//#JN20141222.n

Private aRotina     := {}
//Private cMarca	    := ""	//#JN20141222.o
Private cCadastro   := OemToAnsi("FATURAMENTO - Atualização de Ped.Vendas")
//Private cPerg       := "ATPEDV"	//#JN20141222.o
Private cPerg       := Padr("ATPEDV",Len(SX1->X1_GRUPO))	//#JN20141222.n
//Private _cCliente   := ""	//#JN20141222.o

Private _cArq		:= ""	//#JN20141222.n
Private _cInd		:= ""	//#JN20141222.n
Private _cMarca := GetMark()	//#JN20141222.n
...
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
//_cMarca := GetMark()	//#JN20141222.o

MsAguarde({|| _fSeleDados()},"Aguarde")

//MarkBrow("TRB","OK",,aCampos,,_cMarca)	//#JN20141222.o
MarkBrow("TRB","OK",,aCampos,,_cMarca,'u_MarkAll()')	//#JN20141222.n
...
Ferase(_cArq+".DBF")	//#JN20141222.n
Ferase(_cInd+OrdBagExt())	//#JN20141222.n
...
Local oMark	//#JN20141222.n
Local cFilSB1	:= xFilial("SB1")	//#JN20141222.n
Local cFilSC6	:= xFilial("SC6")	//#JN20141222.n
Local xUltData	:= mv_par03 - 30	//#JN20141222.n
...
While !Eof()
	...
	If 	TRB->OK == _cMarca
		...
		Dbselectarea("SC6")
		//Dbseek(xfilial("SC6") + TRB->PEDIDO)	//#JN20141222.o
		Dbseek(cFilSC6 + TRB->PEDIDO)	//#JN20141222.n
		
		//While .not. eof() .and. ALLTRIM(SC6->C6_NUM)==ALLTRIM(TRB->PEDIDO)	//#JN20141222.o
		While !eof() .and. SC6->C6_FILIAL == cFilSC6 .and.SC6->C6_NUM == TRB->PEDIDO	//#JN20141222.o
			...
			IF  MV_PAR09 > 0
				NewPrcVen := MV_PAR09
			ELSE
				//NewPrcVen := INT(SC6->C6_PRCVEN+(SC6->C6_PRCVEN * MV_PAR05)/100)	//#JN20141222.o
				NewPrcVen := SC6->C6_PRCVEN+ROUND(SC6->C6_PRCVEN * MV_PAR05/100,2)	//#JN20141222.n
			Endif
			...
			IF  xUltData >= MV_PAR03 .AND. xUltData <= MV_PAR04
				...
				//Replace SC6->C6_CODISS  WITH ''	//#JN20141222.o
				Replace SC6->C6_CODISS  WITH Posicione("SB1",1,cFilSB1+SC6->C6_PRODUTO,"B1_CODISS")	//#JN20141222.o
		...
		While xUltData <= MV_PAR04
			...
			//xUltItem := ALLTRIM(STRZERO((VAL(XULTITEM)+1),2))	//#JN20141222.o
			xUltItem := Soma1(xUltItem)	//#JN20141222.n
			...
			If  xUltData <= MV_PAR04
				...
				While SC6->(dbSeek(cFilSC6+cNUM+xUltItem))	//#JN20141222.n
					xUltItem := Soma1(xUltItem)	//#JN20141222.n				
				EndDo	//#JN20141222.n
				
				Reclock("SC6",.T.)
				
				Replace SC6->C6_FILIAL  WITH cFILIAL
				Replace SC6->C6_NUM     WITH cNUM
				Replace SC6->C6_ITEM    WITH xUltItem
				Replace SC6->C6_PRCVEN  WITH NewPrcVen
				Replace SC6->C6_VALOR   WITH NewValor
				Replace SC6->C6_ENTREG  WITH xUltData
				...
				For i:= 1 to len(arquivo)
					IF   !alltrim(arquivo[i,1])$ "C6_ITEM/C6_PRCVEN/C6_VALOR/C6_ENTREG/C6_UM/C6_FILIAL/C6_NUM/C6_NOTA/C6_SERIE/C6_DATFAT/C6_CODISS/C6_QTDENT/C6_QTDEMP"
						replace &(arquivo[i,1]) with arquivo[i,2]
						
						//Gravação do código do ISS
						If alltrim(arquivo[i,1]) == "C6_PRODUTO"	//#JN20141222.n
							Replace SC6->C6_CODISS  WITH Posicione("SB1",1,cFilSB1+arquivo[i,2],"B1_CODISS")	//#JN20141222.n
						EndIf	//#JN20141222.n
...
_cInd := CriaTrab(,.f.)
IndRegua("TRB",_cInd,"PEDIDO+CLIENTE+LOJACLI",,,"Selecionando Dados")    	//#JN20141222.o

cQuery += "SELECT C5_NUM,C5_CLIENTE,C5_LOJACLI,C5_EMISSAO,C5_TIPO,C5_CONDPAG,C5_NOTA, C5_SERIE, A1_NREDUZ "
//cQuery += " FROM "+retsqlname("SC5")+" JOIN "+retsqlname("SA1")+" ON C5_CLIENTE = A1_COD AND C5_LOJACLI = A1_LOJA "    	//#JN20141222.o
//cQuery += " WHERE C5_NUM >= '"+ MV_PAR01 +"' AND C5_NUM <= '"+MV_PAR02+"' "    	//#JN20141222.o
//cQuery += " AND "+retsqlname("SC5")+".D_E_L_E_T_ = ' ' AND "+retsqlname("SA1")+".D_E_L_E_T_ = ' '"    	//#JN20141222.o
cQuery += " FROM "+retsqlname("SC5")+" SC5 INNER JOIN "+retsqlname("SA1")+" SA1 "    	//#JN20141222.n
cQuery += " ON SC5.C5_CLIENTE = SA1.A1_COD AND SC5.C5_LOJACLI = SA1.A1_LOJA AND SA1.D_E_L_E_T_ <> '*' AND SA1.A1_FILIAL = '"+xFilial("SA1")+"' "	//#JN20141222.n
cQuery += " WHERE SC5.D_E_L_E_T_ <> '*' AND SC5.C5_FILIAL = '"+xFilial("SC5")+"' "    	//#JN20141222.n
cQuery += " AND C5_NUM BETWEEN '"+ MV_PAR01 +"' AND '"+MV_PAR02+"' "    	//#JN20141222.n
//Somente pedidos nao encerrados
cQuery += " AND C5_NOTA = '' AND C5_BLQ = '' "    	//#JN20141222.n
cQuery := ChangeQuery(cQuery)    	//#JN20141222.n
...
//TCQUERY cQuery NEW ALIAS "TMP1"	//#JN20141222.o
dbUseArea(.T.,'TOPCONN',TcGenQry(,,cQuery),"TMP1",.T.,.F.)	//#JN20141222.n
...
While !eof()
	...
	RECLOCK("TRB",.T.)
	//TRB->OK			:= ""	//#JN20141222.o
	TRB->PEDIDO		:= TMP1->C5_NUM
	TRB->TIPO		:= TMP1->C5_TIPO
	TRB->CLIENTE	:= TMP1->C5_CLIENTE
	TRB->LOJACLI	:= TMP1->C5_LOJACLI
	TRB->NOMCLI		:= TMP1->A1_NREDUZ
	TRB->CONDPAG	:= TMP1->C5_CONDPAG
	TRB->(MSUNLOCK())
	DbSelectArea("TMP1")
	DbSkip()
Enddo
...
//_sAlias := Alias()	//#JN20141222.o
Local aArea	:= GetArea()	//#JN20141222.n
Local aRegs	:= {}	//#JN20141222.n

dbSelectArea("SX1")
dbSetOrder(1)
//cPerg        := PADR(cPerg,len(sx1->x1_grupo))	//#JN20141222.o
//aRegs :={}	//#JN20141222.o
...
//dbSelectArea(_sAlias)	//#JN20141222.o
RestArea(aArea)	//#JN20141222.n
...
While !TRB->(EOF())
	Reclock("TRB",.F.)
	TRB->OK := IIf(TRB->OK == _cMarca,"",_cMarca)
	TRB->(MsUnlock())
	TRB->(dbSkip())
EndDo
