...
//³Somente mostra a mensagem nos casos onde a chamada da   ³
//³função é efetuada pela rotina de Requerimentos evitando ³
//³a chamada erronea em ourtas rotinas que fazem parte do  ³
//³pacote na configuração do requerimento.                 ³
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
if FunName() == "ACAA410" //Chamada pela rotina de requerimentos
	If Empty( cEmail )
		If lCoSemMail
...
if FunName() == "ACAA410"
	...
	if empty(cEmail)      
		cMens := "o professor "+sra->ra_mat+""
	elseif empty(cEmailCoor)
		cMens := "o coordenador "+jaj->jaj_codcoo+""
	elseif empty(cEmail) .and. empty(cEmailCoor)
	    cMens := "o professor "+sra->ra_mat+" / coordenador "+jaj->jaj_codcoo+""
	endif                                                                        
	if !lRet 
		if Aviso("Aviso","A regra de SMTP não foi executada, pode existir problemas relativos a configuração de SMTP ou acesso ao Servidor.O envio do e-mail não será executado.Deseja prosseguir?",{"Prosseguir","Cancelar"}) == 1
			lRet := .t.
		endif
	else	
		if Aviso("Aviso","Não existe e-mail cadastrado para "+cMens+".O envio de e-mail relativo a Revisão de Prova não será enviado. Deseja prosseguir?",{"Prosseguir","Cancelar"}) == 1
			lRet := .t.
		else
			lRet := .f.	      
		endif
