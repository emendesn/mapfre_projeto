...
Private aPosColRel:={}			//array que ira armazenar a posicao inicial de cada coluna     
Private nPosColRel :=0 
...
cQuery += "SELECT DISTINCT JC7_NUMRA  RA     , JC7_CODPRF MATPRF , JC7_HABILI HABILI, JC7_OUTTUR OUTTUR , JC7_OUTHAB OUTHAB, JC7_TURMA TURMA, "
cQuery += "                JC7_DISCIP DISCIP , JC7_SITUAC SITUAC , X5_DESCRI  SITDIS, JC7_MEDFIM MEDIA , JC7_MEDCON MEDCON, "
cQuery += "                JA2_NOME   ALUNO  , JC7_CODLOC UNIDADE, JC7_CODPRE PREDIO, JC7_ANDAR  ANDAR , JC7_CODSAL SALA  , "
cQuery += "                JAH_CURSO  CURPAD , JC7_CODCUR CURSOF , JC7_PERLET SERIEF, JC7_OUTCUR OUTCUR, JC7_OUTPER OUTPER, "
cQuery += "                JBE_SITUAC MATRICU "
...
cQuery += "  OR JAH_CURSO  Between '"+mv_par03+"' and '"+mv_par04+"' ) "		// Curso De/Ate
...
cQuery += "ORDER BY CURSOF, SERIEF, HABILI, TURMA, OUTCUR, OUTPER, OUTHAB, OUTTUR,DISCIP, MATPRF, ALUNO"
...
Local cDias      := ""
Local cTitCabAv := "" //Descrição da Avaliação
Local cDescCabAux := "" 
Local nPosIniCol := 0
...
While TRB->( !Eof() )
	...
	If lExec
		...
		If 	( iif(Empty(TRB->OUTCUR),TRB->CURSOF+TRB->SERIEF+TRB->HABILI+TRB->TURMA,TRB->OUTCUR+TRB->OUTPER+TRB->OUTHAB+TRB->OUTTUR) # cCurso+cSerie+cHabili+cTurma) .or. ;
			( TRB->DISCIP # cDisciplina ).or. ( TRB->MATPRF # cMatPrf ) .or. ( TRB->SITDIS # cSitDis )
	
			If 	( iif(Empty(TRB->OUTCUR),TRB->CURSOF+TRB->SERIEF+TRB->HABILI+TRB->TURMA,TRB->OUTCUR+TRB->OUTPER+TRB->OUTHAB+TRB->OUTTUR) # cCurso+cSerie+cHabili+cTurma )
	
				cCurso	:= IIF(Empty(TRB->OUTCUR),TRB->CURSOF,TRB->OUTCUR)
				cSerie	:= IIF(Empty(TRB->OUTCUR),TRB->SERIEF,TRB->OUTPER)
				cHabili := IIF(Empty(TRB->OUTCUR),TRB->HABILI,TRB->OUTHAB)   
				cTurma  := IIF(Empty(TRB->OUTCUR),TRB->TURMA,TRB->OUTTUR)
				...
				JAR->( MsSeek(xFilial("JAR")+cCurso+cSerie+cHabili) )
			...
			If 	( TRB->DISCIP # cDisciplina ) .or. ( TRB->TURMA # cTurma ) .or. ( TRB->MATPRF # cMatPrf )
				...
				If (JAR->JAR_APFALT == "1")	// Apontamento Diario
					...
					If Empty(TRB->OUTCUR)
						JBL->(DBSeek(xFilial("JBL")+TRB->CURSOF+TRB->SERIEF+TRB->HABILI+TRB->TURMA+TRB->DISCIP+TRB->MATPRF))
				    Else
					    JBL->(DBSeek(xFilial("JBL")+TRB->OUTCUR+TRB->OUTPER+TRB->OUTHAB+TRB->OUTTUR+TRB->DISCIP+TRB->MATPRF))
				    EndIf
					...
					While 	JBL->JBL_FILIAL == xFilial("JBL") .And.;
							JBL->JBL_CODCUR == cCurso      .And.;
							JBL->JBL_PERLET == cSerie	   .And.;
							JBL->JBL_HABILI == cHabili	   .And.;
							JBL->JBL_TURMA  == cTurma	   .And.;
	 			...
	 			ElseIf (JAR->JAR_APFALT == "2")										// Apontamento Mensal
					...
					nTamCabFal := Len(AllTrim(cDescCab3B)) 
				Else
					cDescCab1B := IIf(Len(cDescCab1B) == 0,Space(9),"|")
				Endif
				...
				dbSetOrder(1)		// Ordem: Curso + Periodo Letivo
				MsSeek( xFilial("JBQ") + cCurso + cSerie + cHabili )
			    
				nqtdnta := 0
				njump := 0
				
				While 	JBQ->JBQ_FILIAL == xFilial("JBQ") .And.;
						JBQ->JBQ_CODCUR == cCurso    .And.;
						JBQ->JBQ_PERLET == cSerie    .And.;
						JBQ->JBQ_HABILI == cHabili   .And. !Eof()
					...
					JDA->(dbSetOrder(1))		// Ordem: Curso+Periodo Letivo+Turma+Disciplina+Avaliacao+Atividade
					
					If Empty(TRB->OUTCUR)
						JDA->(DbSeek(xFilial("JDA")+TRB->CURSOF+TRB->SERIEF+TRB->HABILI+TRB->TURMA+TRB->DISCIP+JBQ->JBQ_CODAVA))
					Else						
						JDA->(DbSeek(xFilial("JDA")+TRB->OUTCUR+TRB->OUTPER+TRB->OUTHAB+TRB->OUTTUR+TRB->DISCIP+JBQ->JBQ_CODAVA))						
					EndIf	
		
					While 	JDA->JDA_FILIAL == xFilial("JDA") .And.;
							JDA->JDA_CODCUR == cCurso	   .And.;
							JDA->JDA_PERLET == cSerie	   .And.;
							JDA->JDA_HABILI == cHabili    .And.;
							JDA->JDA_TURMA  == cTurma	   .And.;
				    ...
				    cCabAtiv	+= Iif(Empty(cCabAtiv),STR0048,STR0047)+" "	//  "Notas"#"Media"      
				   	cTitCabAv     := AllTrim(JBQ->JBQ_DESC)
				  	cTitCabAv     := PadL( Alltrim( cTitCabAv ), Len(cCabAtiv )) 
				   	cDescCab2B += Iif(Len(cCabAtiv)>Len(AllTrim(JBQ->JBQ_DESC)),PadC(AllTrim(JBQ->JBQ_DESC),Len(cCabAtiv),"."),cTitCabAv)+"|"
				    ...
				    cDescCab3B += Iif(Len(cCabAtiv)>Len(AllTrim(JBQ->JBQ_DESC)),cCabAtiv,PadC(cCabAtiv,Len(cTitCabAv)))+"|"
   				    nTamAval   += Len(Iif(Len(cCabAtiv)>Len(AllTrim(JBQ->JBQ_DESC)),cCabAtiv,cTitCabAv))+1
				...
				nPosTotFal := ((219-Len(cDescCab2B)) + 2)+(AT(STR0031,cDescCab2b)-1)
	...
	cDescCabAux := cDescCab3b
	nPosIniCol := 220-Len(cDescCabAux)
	aAdd(aPosColRel, {nPosIniCol}) 
	While At("|",cDescCabAux) <> 0          
		nPosIniCol += At("|",cDescCabAux)
		cDescCabAux := right(cDescCabAux, Len(cDescCabAux) - At("|",cDescCabAux))
		aAdd(aPosColRel, {nPosIniCol}) 		
	End               
	cDescCab3b := Replace(cDescCab3b,"|"," ")
	dbSelectArea("TRB")
	
	//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
	//³ Quebra por Curso+Periodo+Turma+Disciplina ou por estouro do tamanho da pagina ³
	//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
	While 	iif(Empty(TRB->OUTCUR),TRB->CURSOF,TRB->OUTCUR)  == cCurso      .And.;
			iif(Empty(TRB->OUTCUR),TRB->SERIEF,TRB->OUTPER)  == cSerie      .And.;
			iif(Empty(TRB->OUTCUR),TRB->HABILI,TRB->OUTHAB) == cHabili      .And.;
			iif(Empty(TRB->OUTCUR),TRB->TURMA,TRB->OUTTUR)  == cTurma      .And.;
		...
		@ nLin,aPosColRel[nPosColRel][1] PSAY Str(nTotFaltas,TamSX3("JCH_QTD")[1])

		If lAbono   
			nPosColRel++
			@ nLin,aPosColRel[nPosColRel][1] PSAY Str(nTotAbono,TamSX3("JCH_QTD")[1])
		EndIf
		...
		For nInd:=1 To Len(aAvaliacao)
			...
			If Empty(TRB->OUTCUR)
				JDC->(DBSeek(xFilial("JDC")+TRB->RA+TRB->CURSOF+TRB->SERIEF+TRB->HABILI+TRB->TURMA+TRB->DISCIP+aAvaliacao[nInd][1]))
			Else
				JDC->(DBSeek(xFilial("JDC")+TRB->RA+TRB->OUTCUR+TRB->OUTPER+TRB->OUTHAB+TRB->OUTTUR+TRB->DISCIP+aAvaliacao[nInd][1]))			
			EndIf
			If JDC->(!Eof())
				While 	JDC->JDC_FILIAL == xFilial("JDC")  .And.;
						JDC->JDC_NUMRA  == TRB->RA		    .And.;
						JDC->JDC_CODCUR == cCurso		    .And.;
						JDC->JDC_PERLET == cSerie		    .And.;
						JDC->JDC_HABILI == cHabili		    .And.;
						JDC->JDC_TURMA  == cTurma		    .And.;
						JDC->JDC_CODDIS == TRB->DISCIP	    .And.;
						JDC->JDC_CODAVA == aAvaliacao[nInd][1]
					
					nPosColRel++
					If lConceito
					 	@ nLin,aPosColRel[nPosColRel][1] PSAY Iif(Empty(JDC->JDC_NOTA),PadL(cMV_ACCHRNC,3),PadC(JDC->JDC_CONCEI,5))
					Else	
						@ nLin,aPosColRel[nPosColRel][1] PSAY Iif(Empty(JDC->JDC_NOTA) .And. JDC->JDC_COMPAR=="2",PadC(cMV_ACCHRNC,3),Transform(JDC->JDC_NOTA,ACPicture("JDC_NOTA") ) ) //Str(JDC->JDC_NOTA,iif(JDC->JDC_NOTA>=10, 5, 4),2))
					EndIf	
			...
			Else
				...
				For nAtiv:=1 To Len(aAtividades)
					...
					If aAtividades[nAtiv] == aAvaliacao[nInd][1]
						nPosColRel++
						@ nLin,aPosColRel[nPosColRel][1] PSAY PadL(cMV_ACCHRNC,5)
					EndIf
			...
			JBS->(dbSetOrder(2)) 		//	Ordem: Ra+Curso+Periodo+Turma+Diciplina+Avaliacao
			
			If Empty(TRB->OUTCUR)
				JBS->(DBSeek(xFilial("JBS")+TRB->RA+TRB->CURSOF+TRB->SERIEF+TRB->HABILI+TRB->TURMA+TRB->DISCIP+aAvaliacao[nInd][1]))
			Else
				JBS->(DBSeek(xFilial("JBS")+TRB->RA+TRB->OUTCUR+TRB->OUTPER+TRB->OUTHAB+TRB->OUTTUR+TRB->DISCIP+aAvaliacao[nInd][1]))			
			EndIf	
			...
			If JBS->(!Eof())
				...
				If !Empty(JBS->JBS_DTCHAM) .and. aAvaliacao[nInd][2] == "S"
					nPosColRel++
				  	@ nLin,aPosColRel[nPosColRel][1] PSAY PadC(cMV_ACCHRNC,5)						
				EndIf

				If lAbono
					nPosColRel++
					If lConceito
					  	@ nLin,aPosColRel[nPosColRel][1] PSAY Iif(Empty(JBS->JBS_CONCEI),PadC(cMV_ACCHRNC,5),PadC(JBS->JBS_CONCEI,5))
					Else
						@ nLin,aPosColRel[nPosColRel][1] PSAY Iif(Empty(JBS->JBS_NOTA),PadC(cMV_ACCHRNC,5),Transform(JBS->JBS_NOTA,ACPicture("JBS_NOTA") ) ) //Alltrim(Str(JBS->JBS_NOTA,iif(JBS->JBS_NOTA>=10, 5, 4),iif(JBS->JBS_NOTA>=10, 1, 2))))
					EndIf
				Else            
					nPosColRel++
					If lConceito
					  	@ nLin,aPosColRel[nPosColRel][1] PSAY Iif( Empty( JBS->JBS_CONCEI), PadC( cMV_ACCHRNC, 5 ), PadC(JBS->JBS_CONCEI,5 ) )
					Else
						@ nLin,aPosColRel[nPosColRel][1] PSAY Iif( JBS->JBS_COMPAR == "2", PadC( cMV_ACCHRNC, 5 ), Transform(JBS->JBS_NOTA,ACPicture("JBS_NOTA") ) )//Alltrim(Str(JBS->JBS_NOTA, iif(JBS->JBS_NOTA>=10, 5, 4), iif(JBS->JBS_NOTA>=10, 1, 2) ) ))
					EndIf
				EndIf
		...
		Else
			aEncNot := ACEncNota( TRB->RA, TRB->OUTCUR, TRB->OUTPER, TRB->OUTTUR, TRB->DISCIP, TRB->MEDIA, TRB->MEDCON, TRB->SITUAC )		
		EndIf          
		nMedia  := aEncNot[1,1]
		cMedCon := aEncNot[1,2]
		cSituac := aEncNot[1,3]
		...
		If lConceito  
			nPosColRel++
			@ nLin,aPosColRel[nPosColRel][1] PSAY if(Empty(cMedCon),NotCon(nMedia),cMedCon)
		Else
        	If !Empty(nMedia)
        		nPosColRel++
				@ nLin,aPosColRel[nPosColRel][1] PSAY Transform(nMedia,ACPicture("JBS_NOTA"))//Alltrim(Str(nMedia,iif(nMedia>=10, 5, 4),iif(nMedia>=10, 1, 2))) //Picture "@E 99.99"
			Else	
        		nPosColRel++
				@ nLin,aPosColRel[nPosColRel][1] PSAY padl(cMV_ACCHRNC,5) //Picture "@E 99.99"
			EndIf
		EndIf
		...
		nPosColRel++
		@ nLin,aPosColRel[nPosColRel][1] PSAY cDescBox	// Situacao do aluno na disciplina		
	...
	If 	(IIF(EMPTY(TRB->OUTCUR),TRB->CURSOF+TRB->SERIEF+TRB->HABILI+TRB->TURMA+TRB->DISCIP+TRB->MATPRF,TRB->OUTCUR+TRB->OUTPER+TRB->OUTHAB+TRB->OUTTUR+TRB->DISCIP+TRB->MATPRF )# cCurso+cSerie+cHabili+cTurma+cDisciplina+cMatPrf )
...
Local cDCurso  := STR0016+AllTrim(TRB->CURPAD)+"/"+IIf(Empty(TRB->OUTCUR),TRB->CURSOF+"/"+TRB->HABILI,TRB->OUTCUR+"/"+TRB->OUTHAB) +" - "+AllTrim(Posicione("JAF",1,xFilial("JAF")+JAH->JAH_CURSO+JAH->JAH_VERSAO,"JAF_DESMEC"))+" ("+Capital( Tabela( "F5", JAH->JAH_TURNO ) )+") "+STR0027+ IIf(Empty(TRB->OUTCUR),TRB->SERIEF,TRB->OUTPER)+"ª "+IIf(Empty(TRB->OUTCUR),TRB->TURMA,TRB->OUTTUR)
Local cCargaH  := ''
...
If (JAR->JAR_APFALT == "1")	// Apontamento Diario
	...
	If nLinfal > 0
		...
		If lExec
			...
			For nInd:= (nLinFal+1) To Len(aCabFaltas)
	
				If nCab == 1
					cDescCab1B := PadR(aCabFaltas[nInd],nTamCabFal)+"|"+Iif(labono,space(19),Space(10))+cDescCab1B
				ElseIf nCab == 2
		...
		nPosIniFaC :=220-Len(cDescCab3B)
		For nInd:=1 To nLinFal
			@ nLin,nPosIniFaC PSAY PadR(aCabFaltas[nInd],nTamCabFal)	
			nLin++
		Next nInd
...
Else     
	If lAbono
		cDescCab1B := Space(10)+cDescCab1B                
	EndIf	
EndIf
nPosIniFaC :=220-Len(cDescCab3B)
If !Empty(cDescCab1A+cDescCab1B)
	@ nLin, 000 PSAY Replace(cDescCab1A,"|"," ")
	@ nLin, nPosIniFaC PSAY Replace(cDescCab1B,"|"," ")
	nLin++                               
EndIf
If !Empty(cDescCab2A+cDescCab2B)
	@ nLin, 000 PSAY Replace(cDescCab2A,"|", " ")
	@ nLin,nPosIniFaC PSAY Replace(cDescCab2B,"|"," ")
	nLin++                               
EndIf
If !Empty(cDescCab3A+cDescCab3B)
	@ nLin, 000 PSAY Replace(cDescCab3A,"|", " ")
	@ nLin,nPosIniFaC PSAY Replace(cDescCab3B,"|", " ")
	nLin++                               
EndIf
@ nLin,000 PSAY __PrtThinLine()
nLin += 1
...
JAR->( dbSetOrder(1) )	// Ordem: Curso + Periodo Letivo
IF EMPTY(TRB->OUTCUR)
	JAR->( DBSeek(xFilial("JAR")+TRB->CURSOF+TRB->SERIEF+TRB->HABILI) )
Else
	JAR->( DBSeek(xFilial("JAR")+TRB->OUTCUR+TRB->OUTPER+TRB->OUTHAB) )
EndIf
...
Local nLinFal		:= 0
Local nColIni 		:= 0
Local cCurso		:= iif(Empty(TRB->OUTCUR),TRB->CURSOF,TRB->OUTCUR)
Local cPerlet		:= iif(Empty(TRB->OUTCUR),TRB->SERIEF,TRB->OUTPER)
Local cHabili		:= iif(Empty(TRB->OUTCUR),TRB->HABILI,TRB->OUTHAB)
Local cTurma		:= iif(Empty(TRB->OUTCUR),TRB->TURMA,TRB->OUTTUR) 
...
JCH->(DBSeek(xFilial("JCH")+TRB->RA+cCurso+cPerlet+cHabili+cTurma+TRB->DISCIP))

While 	JCH->JCH_FILIAL+JCH->JCH_NUMRA+JCH->JCH_CODCUR+JCH->JCH_PERLET+JCH->JCH_HABILI+JCH->JCH_TURMA+JCH->JCH_DISCIP ==;
		xFilial("JCH")+TRB->RA+cCurso+cPerlet+cHabili+cTurma+TRB->DISCIP	
...
If (JAR->JAR_APFALT # "3")	// Apontamento Dario ou Mensal
	...
	For nInd:=1 To Len(aFaltas)
		...
		nColFal += ( (218-Len(cDescCab1B) - nColIni) + If(JAR->JAR_APFALT == "1",4,3)) 
