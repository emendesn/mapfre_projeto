...
AADD(TB_Campos,{"WKPRECO" ,"",STR0006+IF(SW2->W2_PARID_U==0,SW2->W2_MOEDA,"US$"),E_TrocaVP(1,_PictPrUn)}) //"UNIT PRICE "
AADD(TB_Campos,{"WKAMOUNT","",STR0007+IF(SW2->W2_PARID_U==0,SW2->W2_MOEDA,"US$"),E_TrocaVP(1,_PictPrUn)}) //"AMOUNT "
aCAMPOS:= {}
aHEADER:= {}
...
WHILE .T.
  ...
  IF Work->(LASTREC()) > 0
   ...
   nLin:=1.4 ; nColS1:=1.0  ; nColG1:=7.0 ; nColS2:=17.0 ; nColG2:=21.0
      ...
      oSend(TSay(),"New",nLin         ,nColS1,{|| OemToAnsi(STR0022)},oDlg,,oFntdlg,,,,.F. ) //"Nº do P.O.:"
   V:=oSend(TGet(),"New",nLin         ,nColG1 ,bSetGet(TPO_Num)   ,oDlg,40,8,;
   ...
   V:=oSend(TGet(),"New",nLin         ,nColG2 ,bSetGet(TImport)   ,oDlg,10,8,;
      ...
      oSend(TSay(),"New",nLin:=nLin+1 ,nColS1,{|| STR0014            },oDlg,,oFntdlg,,,,.F. ) //"Nro. Prof."
   V:=oSend(TGet(),"New",nLin         ,nColG1 ,bSetGet(TNr_Pro)   ,oDlg,50,8,;
      ...
      oSend(TSay(),"New",nLin         ,nColS2,{|| STR0015                 },oDlg,,oFntdlg,,,,.F. ) //"Date "
   V:=oSend(TGet(),"New",nLin         ,nColG2 ,bSetGet(TDt_Pro)   ,oDlg,40,8,;
   ...
   V:=oSend(TGet(),"New",nLin         ,nColG2 ,bSetGet(TAgente)   ,oDlg,10,8,;
      ...
      oSend(TSay(),"New",nLin:=nLin+1 ,nColS1,{|| STR0018               },oDlg,,oFntdlg,,,,.F. ) //"Country"
   V:=oSend(TGet(),"New",nLin         ,nColG1 ,bSetGet(TPais)   ,oDlg,50,8,,,,,oFntDlg,,,.F.,,,,,,,,,"SW2")
      oSend(V,"DISABLE")
   ...
   V:=oSend(TGet(),"New",nLin         ,nColG2 ,bSetGet(TTipo_Emb)   ,oDlg,10,8,;
      ...
      oSend(TSay(),"New",nLin, nColS4,{|| STR0023                  },oDlg,,oFntdlg,,,,.F. ) //"Total FOB"
      oSend(TSay(),"New",nLin, nColS5,{||  MTotal                      },oDlg,_PictPrUn,oFntdlg,,,,.F. )
     ...
     @ 25,213 RADIO aRad1 VAR nTipoImp

     oSend( SButton(), "New", 18,(oDlg:nClientWidth-4)/2-30 , 6 ,{||nOpcao:=1,oSend(oDlg,"End") }, oDlg, .T.,,)
     oSend( SButton(), "New", 31,(oDlg:nClientWidth-4)/2-30 , 2 ,{||nOpcao:=0,oSend(oDlg,"End") }, oDlg, .T.,,)
...
WHILE !SW3->(EOF()) .AND. SW3->W3_PO_NUM==TPO_Num .AND. SW3->W3_FILIAL==xFilial("SW3")
   ...
   Work->WKNET_W := SB1->B1_PESO * SW3->W3_QTDE
   Work->WKUNI   := IF(! EMPTY(SA5->A5_UNID),SA5->A5_UNID,SB1->B1_UM)
...
AVPRINT oPrn NAME STR0043 //"Proforma Invoice"
   ...
   AVPAGE
        ...
        WHILE !SW3->(EOF()) .AND. SW3->W3_FILIAL==xFilial("SW3") .AND. ;
           ...
           nNetWeight := nNetWeight + val(trans(SW3->W3_QTDE*SB1->B1_PESO,"99999999999.999"))
           ...
           PARTE2:=2
           PO557VERFIM()
           oPrn:Say( nLinha:=nLinha+50,420  ,STR0045+alltrim(trans(SW3->W3_QTDE*SB1->B1_PESO,"@ 99,999,999.999"+" KGS")),aFontes:TIMES_NEW_ROMAN_12) //"NET WEIGHT:"
        ...
        ELSE
           ...
           ELSEIF SW2->W2_TIP_COM $ "2,3"
              ...
              EndIf
              oPrn:Say( nLinha:=nLinha+50 ,110 ,"US$ "+trans(nVal_Com,"@ 999,999,999.99"),aFontes:TIMES_NEW_ROMAN_12 )
...
oPrn:Say( nLinha+230 , 130  , STR0068 + Trans(SYT->YT_CGC,"@R 99.999.999/9999-99"),aFontes:TIMES_NEW_ROMAN_12 ) //"C.G.C.: "
