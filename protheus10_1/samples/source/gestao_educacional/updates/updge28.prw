...
±±³Fun‡…o    ³ UpdGE28  ³ Autor ³ karen honda          ³ Data ³ 18/jul/07 ³±±
...
±±³          ³ melhoria do controle de hora aula por periodo letivo.      ³±±
...
User Function UpdGE28() //Para maiores detalhes sobre a utilizacao deste fonte leia o boletim
...
nPos := 0
SX3->( MsSeek( "JAR" ) )
While SX3->( ! Eof() .And. Left( X3_ARQUIVO, 3 ) == "JAR" )
	nPos++
	SX3->( dbSkip() )
Enddo                         

nPos++
aAdd(aSX3,{ "JAR",;								//Arquivo
			...
			0,;									//Decimal
			"Tipo de Hora",;						//Titulo
			"Tipo de Hora",;						//Titulo SPA
			"Tipo de Hora",;			    		//Titulo ENG
			"Tipo Hora da Carga Horaria",;				//Descricao
			"Tipo Hora da Carga Horaria",;				//Descricao SPA
			"Tipo Hora da Carga Horaria",;				//Descricao ENG
			"",;								//Picture
			'A210VlTpHr()',;					//VALID
			cUsadoOpc,;							//USADO
			"",;								//RELACAO
			...
			1,;									//NIVEL
			cReservOpc,;						//RESERV
			"","N","","N","A",;					//CHECK, TRIGGER, PROPRI, BROWSE, VISUAL
			"R","","",;							//CONTEXT, OBRIGAT, VLDUSER
			"1=Aula;2=Relogio","","",;				//CBOX, CBOX SPA, CBOX ENG
...
aAdd(aSX3,{ "JAR",;								//Arquivo
			...
			0,;									//Decimal
			"Qtd. Minutos",;						//Titulo
			"Qtd. Minutos",;						//Titulo SPA
			"Qtd. Minutos",;			    		//Titulo ENG
			"Minutos equiv. hora aula",;				//Descricao
			"Minutos equiv. hora aula",;				//Descricao SPA
			"Minutos equiv. hora aula",;				//Descricao ENG
			"999",;								//Picture
			'M->JAR_HRAULA > 0',;					//VALID
			cUsadoOpc,;							//USADO
			"0",;								//RELACAO
			"",;								//F3
			1,;									//NIVEL
			...
			"","","",;				//CBOX, CBOX SPA, CBOX ENG
		   	"","GdFieldGet('JAR_TIPHOR') == '1'","","","","S"})				//PICTVAR, WHEN, INIBRW, SXG, FOLDER, PYME */
