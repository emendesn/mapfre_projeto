...
User Function NFDETECT()        // incluido pelo assistente de conversao do AP5 IDE em 01/08/01
...
SetPrvt("I,NPELEM,_CLASFIS,XPESO_LIQUID,XPED")
...
SetPrvt("XV_ICMS_SOL,NCONT,NCOL,NTAMOBS,NAJUSTE,BB,xDESP,xQTDNF")
SetPrvt("NTOT_PA,NTOT_ISS,NTOT_IR,NTOT_MO,XVALISS,XVAL_IRRF,NLIMITE,XCOD_CLASS,X,Y,A,B")
SetPrvt("XTRANSP,XNUMDI,XQTDVOL,XESPECIE,XMARCA,XNUMVOL,XPBRUTO,XPLIQUI,XVALCOF,XVALPIS,XOBSIMP,XTAXASISCO") 

/*/
ÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜ
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÚÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄ¿±±
±±³Programa  ³  NfCesvi ³ Autor ³   Anderson Rodrigues  ³ Data ³ 31/07/01 ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
±±³Descri‡…o ³ Nota Fiscal de Entrada/Saida e Servicos                    ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
±±³Uso       ³ Especifico para Cesvi do Brasil                            ³±±
...
Else
	...
	While !eof() .and. SF1->F1_DOC <= mv_par02 .and. SF1->F1_SERIE == mv_par03 .and. lContinua
		...
		XTRANSP      := SF1->F1_X_TRANS
		XNUMDI       := SF1->F1_X_DI
		XESPECIE     := SF1->F1_X_TPVOL
		XMARCA       := SF1->F1_X_MARCA
		XNUMVOL      := SF1->F1_X_NUMVO
		XPBRUTO      := SF1->F1_X_PBRUT
		XPLIQUI      := SF1->F1_X_PLIQ 
		XTPFRETE     := SF1->F1_X_TPFRE
		XTAXASISCO   := SF1->F1_X_SISCO
		XOBSIMP      := SF1->F1_X_OBS
		IF  !EMPTY(XNUMDI)
		    XQTDVOL   := SF1->F1_X_QTVOL 
		    XVALCOF  := SF1->F1_VALIMP5
		    XVALPIS  := SF1->F1_VALIMP6
		ENDIF
		...
		while !eof() .and. SD1->D1_DOC==xNUM_NF
			...
			AADD(xNOTA_ORI ,SD1->D1_NFORI)             //Nota Fiscal Original
			XVALCOF  := XVALCOF + SD1->D1_VALIMP5
...
For I:=1 to nTamDet
	...
	IF I<= Len(xCOD_PRO)
		...
		DbSeek(xFilial("SF4")+xTes[I],.F.)
			...
			@ nLin, 003  PSAY Alltrim(xCOD_PRO[I])
			@ nLin, 020  PSAY Alltrim(xDESCRICAO[I])
			@ nLin, 078  PSAY xCLFISCAL[I] + "-"
			@ nLin, 080  PSAY Transform(xCLAS_FIS[I],"@r 99.99.99.99.99")
			@ nLin, 106  PSAY xUNID_PRO[I]
			@ nLin, 115  PSAY xQTD_PRO[I]           Picture "@E 999,999"
			@ nLin, 125  PSAY xPRE_UNI[I]           Picture "@E 99,999,999.99"
			@ nLin, 150  PSAY xVAL_MERC[I]          Picture "@E 99,999,999.99"
			@ nLin, 169  PSAY xICMS[I]              Picture "@E 999"
			@ nLin, 174  PSAY xIPI [I]              Picture "@E 999"
			@ nLin, 179  PSAY xVAL_IPI[I]           Picture "@E 99,999,999.99"
			J:=J+1
			nLin := nLin + 1
			xQTDNF := xQTDNF + xQTD_PRO[I]
	Endif
...
nlin:=nlin+1
@ nLin, nCol PSAY UPPER(SUBSTR(xMENSAGEM,51,nTamObs))
nlin:=nlin+1
@ nLin, nCol PSAY UPPER(SUBSTR(xMENSAGEM,101,nTamObs))
Return
...
@ 01, 187 PSAY xNUM_NF               // Numero da Nota Fiscal
If mv_par04 == 1
	@ 02, 162 PSAY "X"
Else
	@ 02, 148 PSAY "X"
Endif
IF  ALLTRIM(XMENSDEV) <> "" //MENSAGEM PARA NF DE DEVOLUCAO DE VENDA
    XMENSAGEM := "REF. NOTA FISCAL "+XMENSDEV
ENDIF    
IF  ALLTRIM(XOBSIMP) <> ""
    XMENSAGEM := XOBSIMP
ENDIF    
@ 07, 003 PSAY UPPER(SUBSTR(xMENSAGEM,1,nTamObs))
@ 07, 064 PSAY XNATUREZA       // Texto da Natureza de Operacao
@ 07, 101 PSAY xCF[1] Picture"@R 9.999" // Codigo da Natureza de Operacao
//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
//³ Impressao dos Dados do Cliente      ³
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
@ 08, 003 PSAY UPPER(SUBSTR(xMENSAGEM,46,nTamObs))
IF!EMPTY(xCOD_MENS) 
  DBSELECTAREA("SM4")
  DBSETORDER(1)
  IF DBSEEK(XFILIAL("SM4")+xCOD_MENS)
  	  xMENSAGEM := ALLTRIM(SM4->M4_FORMULA)
  ELSE
	  xMENSAGEM := " "
  ENDIF           
ELSE
	xMENSAGEM := " "
ENDIF
IF  !EMPTY(ALLTRIM(XNUMDI))
     @ 09,003 PSAY "DI: " + ALLTRIM(XNUMDI)
     @ 10,003 PSAY "PIS: "
     @ 10,012 PSAY XVALPIS PICTURE "@E 999,999,999.99"
ELSE     
	@ 09, 003 PSAY UPPER(SUBSTR(xMENSAGEM,1,nTamObs))
	@ 10, 003 PSAY UPPER(SUBSTR(xMENSAGEM,46,nTamObs))     
ENDIF
	
@ 10, 064 PSAY xNOME_CLI              //Nome do Cliente
IF !EMPTY(xCGC_CLI)                   // Se o C.G.C. do Cli/Forn nao for Vazio
	@ 10, 147 PSAY xCGC_CLI Picture"@R 99.999.999/9999-99"
Else
	@ 10, 145 PSAY " "                // Caso seja vazio
Endif
@ 10, 185 PSAY xEMISSAO                                 // Data da Emissao do Documento
IF  XVALCOF <> 0
     @ 11,003 PSAY "COFINS: "
     @ 11,012 PSAY XVALCOF PICTURE "@E 999,999,999.99"
ELSE     
	@ 11, 003 PSAY UPPER(SUBSTR(xMENSAGEM,91,nTamObs))
ENDIF
IF  XTAXASISCO <> 0
    @ 12,003 PSAY "TAXA SISCOMEX: "
    @ 12,017 PSAY XTAXASISCO PICTURE "@E 999,999,999.99"
ELSE    
	@ 12, 003 PSAY UPPER(SUBSTR(xMENSAGEM,137,nTamObs))
ENDIF	
@ 12, 063 PSAY xEND_CLI                                 // Endereco
@ 12, 127 PSAY xBAIRRO                                  // Bairro
@ 12, 164 PSAY xCEP_CLI Picture"@R 99999-999"           // CEP
@ 12, 182 PSAY xEMISSAO                  
@ 13, 003 PSAY UPPER(SUBSTR(xMENSAGEM,182,nTamObs))
@ 14, 003 PSAY UPPER(SUBSTR(xMENSAGEM,228,nTamObs))
@ 14, 063 PSAY xMUN_CLI                               // Municipio
@ 14, 107 PSAY xTEL_CLI                               // Telefone/FAX
@ 14, 139 PSAY xEST_CLI                               // U.F.
@ 14, 145 PSAY xINSC_CLI                              // Insc. Estadual
...
If xValIss > 0
  nLin := 35
  @ nLin, 125 PSAY xVALISS Picture "@E@Z 999,999,999.99"  
Endif
//If  xBase_Iss > 0
//   nLin := 35
...
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
IF xBase_Icms > 0
   @ 38, 003  PSAY xBASE_ICMS  Picture"@E@Z 999,999,999.99"  // Base do ICMS
Endif
If xVALOR_ICMS > 0
   @ 38, 025  PSAY xVALOR_ICMS Picture"@E@Z 999,999,999.99"  // Valor do ICMS
Endif

_nValProd := xVALOR_MERC - xBase_Iss
If xBSICMRET > 0 
   @ 38, 064  PSAY xBSICMRET  Picture "@E@Z 999,999,999.99"  // Base ICMS Ret.
Endif
If xICMS_RET > 0
   @ 38, 080  PSAY xICMS_RET  Picture "@E@Z 999,999,999.99"  // Valor  ICMS Ret.
Endif
If _nValProd > 0
   @ 38, 115  PSAY _nValProd  Picture "@E@Z 999,999,999.99"  // Valor Tot. Prod.
Endif
If xFRETE > 0
   @ 40, 003  PSAY xFRETE     Picture "@E@Z 999,999,999.99"  // Valor do Frete
Endif
If xSEGURO > 0
   @ 40, 025  PSAY xSEGURO    Picture "@E@Z 999,999,999.99"  // Valor Seguro
Endif             
IF	xDESP > 0
   @ 40, 060  PSAY xDESP      Picture "@E@Z 999,999,999.99"  // Valor Seguro
ENDIF
If xVALOR_IPI > 0                      
   @ 40, 080  PSAY xVALOR_IPI Picture "@E@Z 999,999,999.99"  // Valor do IPI
Endif
If xTOT_FAT > 0
   @ 40, 115  PSAY xTOT_FAT   Picture "@E@Z 999,999,999.99"  // Valor Total NF
Endif

@ 43, 003  PSAY	xNOME_TRANSP 
@ 43, 080 PSAY IIF(xTPFRETE=="C","1","2")
@ 43, 113  PSAY	xCGC_TRANSP   Picture "@R 99.999.999/9999-99" 
@ 45, 003  PSAY	xEND_TRANSP  
@ 45, 066  PSAY	xMUN_TRANSP  
@ 45, 099  PSAY	xEST_TRANSP  
IF  XQTDVOL > 0
	@ 47, 003  PSAY xQTDVOL Picture "@E 999,999.9999"                                                            
ELSE	
	@ 47, 003  PSAY xQTDNF Picture "@E 999,999.9999"  
ENDIF	
@ 47, 025  PSAY	Subs(xESPECIE,1,20)                         // Especie 1 no Pedido
@ 47, 050  PSAY Subs(XMARCA,1,10)
@ 47, 060  PSAY Subs(XNUMVOL,1,10)
@ 47, 097  PSAY	xPBRUTO Picture "@E 999,999.9999"            // Peso Bruto
@ 47, 118  PSAY	xPLIQUI Picture "@E 999,999.9999"            // Peso Liquido
@ 47, 205 PSAY xNUM_NF              // Numero da Nota Fiscal

@ 49, 000 PSAY chr(18)              // Descompressao de Impressao
